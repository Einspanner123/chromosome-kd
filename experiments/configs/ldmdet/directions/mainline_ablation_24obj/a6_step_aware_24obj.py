"""24obj 方向 C: Step-aware Embedding
...
组件:
  ...
  + step_proj 零初始化, 确保加载 +DPM-Solver++ 预训练权重时行为不变 (step_emb≡0)
  + solver_type='dpm_solver_pp', sampling_timesteps=4 (与 +DPM-Solver++ 一致)
...
训练:
  ...
  - 从 +DPM-Solver++ checkpoint 初始化 (step_proj=0, 不破坏预训练)
...
对照: +DPM-Solver++ (DPM-Solver++ 4步, 无 step-aware) → 验证 step-aware 的增益
