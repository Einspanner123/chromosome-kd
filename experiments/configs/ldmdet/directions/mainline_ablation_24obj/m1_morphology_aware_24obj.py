...
组件:
  + shape_attention = MorphologyAwareRoIEncoder (填充现有 hook)
    ...
    - fuse 零初始化: 初始 morph_emb≡0, 加载 A4 预训练时行为不变
  - 其余与 A4 完全一致 (DPM-Solver++ 4步, RF, AdaLN-Zero, StochOT eps5)
...
训练:
  ...
  - mmengine load_from 默认 strict=False, M1 新参数 (h_conv/v_conv/fuse/norm)
    不在 A4 checkpoint 中, 保持初始化值 (fuse=0 → 恒等)

重点观察:
  - C 组 (C6-C12, 亚中着丝粒, 臂长比是主要区分依据)
  - G/Y 组 (尺寸相近, mAP 0.789/0.781, 需形态区分)
  - 整体 mAP 是否超越 A4 baseline (0.863)

对照: A4 (DPM-Solver++, 无形态感知) → 验证 M1 形态增强的增益
...
# === 从 A4 checkpoint 微调 ===
# mmengine load_from 默认 strict=False: A4 权重加载, M1 新参数保持初始化 (fuse=0)
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            ...
            description='24obj M1: 形态感知 RoI 编码器 (零初始化残差, 从 A4 微调 30ep) | bs=2, lr=1e-5',
