"""24obj 主路线消融实验 +DPM-Solver++ (验证采样器效果)

目的: 在 +Stoch. Coupling 完整 SOTA 基础上, 将 Heun 替换为 DPM-Solver++
组件:
  + solver_type='dpm_solver_pp' (DPM-Solver++ 高阶 ODE 求解器)
  + sampling_timesteps=4 (与 +Stoch. Coupling Heun 4步保持一致, 公平对比采样器)
  - RF + AdaLN + StochOT eps5 保持不变

对照: +Stoch. Coupling (Heun 4步) → 验证 DPM-Solver++ vs Heun 的采样器效果
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            ...
            description='24obj 主路线消融 +DPM-Solver++ 4步 (vs +Stoch. Coupling Heun 4步) | bs=8, 150ep',
