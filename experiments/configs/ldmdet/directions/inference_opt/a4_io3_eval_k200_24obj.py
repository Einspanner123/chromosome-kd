"""+DPM-Solver++ + IO3 推理评估 — K=200 (在 +DPM-Solver++ checkpoint 上直接评估, 不训练)

目的: 测试 +DPM-Solver++ (DPM-Solver++) 叠加 Top-K 剪枝 K=200 的 mAP 与速度
方法: 加载 a4_dpm_pp checkpoint, 推理时启用 topk_pruning K=200
理论: +DPM-Solver++ 每步 1 次模型调用, 叠加 K=200 后续步 Self-Attn 加速 6.25x
预期: mAP 下降 <0.005, 延迟低于 +DPM-Solver+++K300
...
# IO3: Top-K 剪枝 K=200 (在 +DPM-Solver++ 基础上)
model = dict(
    bbox_head=dict(
