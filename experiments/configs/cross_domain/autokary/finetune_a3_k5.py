"""跨域 Few-shot 微调: LDMDet A3 (RF+Heun+AdaLN+StochOT eps5) on AutoKary k=5
...
理论对比:
  - A3 (StochOT) vs A2 (Random): 验证 StochOT 在少样本下是否有优势
  - R_idx 维度依赖假说: d=4, K≈46 → 多样性主导 → StochOT 净效果趋零

Usage:
    python experiments/runners/train.py \\
        experiments/configs/cross_domain/autokary/finetune_a3_k5.py
"""
import glob
...
# === Load 24obj pretrained checkpoint (Sinkhorn seed_42 = A3 equivalent) ===
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            ...
            experiment_name='finetune_a3_k5',
            description='Few-shot k=5: LDMDet A3 (StochOT+AdaLN) 24obj→AutoKary | lr=5e-6, 50ep',
