"""Few-shot target finetune: DINO R50 on chromo k=10.
...
_base_ = [
    '../../../../projects/LDMDet/configs/benchmark_24obj/dino_r50.py'
]
...
ckpt_pattern = (
    'work_dirs/few_shot/source_pretrain_dino_r50_24obj/'
    'best_coco_bbox_mAP_*.pth'
)
_ckpts = sorted(glob.glob(ckpt_pattern))
load_from = _ckpts[-1] if _ckpts else ckpt_pattern

# === Finetune optimization (lr = pretrain lr * 0.1 = 2.5e-06) ===
# Keep paramwise_cfg with backbone lr_mult=0.1 from pretrain config.
optim_wrapper = dict(optimizer=dict(lr=2.5e-06))
    ...
    vis_backends=[
        ...
        dict(
            ...
            init_kwargs=dict(
                ...
                experiment_name='target_finetune_dino_r50_k10',
                description='Few-shot target finetune: DINO R50 on '
                'chromo k=10 | bs=2, 50ep',
            ),
        ),
    ],
)
