    ...
    def before_run(self, runner):
        ...
        for src, dst in self.pairs:
            ...
            if not os.path.isdir(abs_src):
                ...
                continue
            runner.logger.info(
                f'Backing up {src} from {abs_src} to {dst_path}'
            )
...
class PredictionVisHook(Hook):
    ...
    def before_val(self, runner):
        ...
        if ref_path.exists():
            ...
            runner.logger.info(
                f'[VisHook] loaded {len(self._ref_images)} reference images from {ref_path}'
            )
            return
        ...
        if val_loader is None:
            runner.logger.warning(
                '[VisHook] no val dataloader, skip reference selection'
            )
            self._ref_images = []
            return
        ...
        if isinstance(outputs, list):
            for i, out in enumerate(outputs):
                if i >= len(data_samples):
                    break
                self._capture_if_reference(
                    data_samples[i], out, ref_set, epoch
                )
        elif hasattr(outputs, 'pred_instances') and data_samples:
            self._capture_if_reference(
                data_samples[0], outputs, ref_set, epoch
            )
    ...
    def after_val_epoch(self, runner, metrics=None):
        ...
        for item in frames:
            ...
            if (
                isinstance(scale_factor, (np.ndarray, list, tuple))
            ...
            if item['gt_bboxes'] is not None:
                for bbox, label in zip(item['gt_bboxes'], item['gt_labels']):
                    x1, y1, x2, y2 = [
                        int(v / f) for v, f in zip(bbox, [sx, sy, sx, sy])
                    ]
        ...
        runner.logger.info(
            f'[VisHook] epoch {epoch}: {len(frames)} frames saved'
        )
        ...
        try:
            ...
            for p in jpgs:
                ...
                if img is not None:
                    ...
                    swanlab.log(
                        {f'predictions/{p.stem}': swanlab.Image(img)},
                        step=epoch,
                    )
        except Exception:
            pass
    ...
    def after_train_iter(
        ...
        for name, param in model.named_parameters():
            if param.requires_grad:
                if self.log_norm:
                    layer_norms[f'weights_norm/{name}'] = param.data.norm(
                        2
                    ).item()
        ...
        if self.log_heatmap and grad_energies and runner.rank == 0:
            try:
                ...
                swanlab.log(
                    {'gradient_activity': swanlab.Image(fig)}, step=runner.iter
                )
                plt.close(fig)
                ...
                if all_grads:
                    ...
                    ax2.hist(
                        concat, bins=50, color='steelblue', edgecolor='white'
                    )
                    ax2.set_title(
                        f'Gradient Distribution (Iter {runner.iter})'
                    )
                    ax2.set_xlabel('Gradient Value')
                    ax2.set_ylabel('Count')
                    swanlab.log(
                        {'gradient_distribution': swanlab.Image(fig2)},
                        step=runner.iter,
                    )
                    plt.close(fig2)
            except Exception:
