#!/usr/bin/env python3
"""
Comprehensive multi-dimensional comparison of two chromosome detection datasets.

Datasets:
  1. Chromosome20240904_NoAug_NoResize_coco  (24 classes, karyoflow)
  2. 24_chromosomes_object                    (24 classes, standard COCO)

Outputs: analysis/dataset_viz/figs/*.png
"""
...
DATASETS = {
    'Chromosome20240904': Path('data/Chromosome20240904_NoAug_NoResize_coco'),
    '24_chromosomes_object': Path('data/24_chromosomes_object'),
}


# ── Data Loading ────────────────────────────────────────────────────
def load_coco_annotations(data_dir: Path, name: str):
    """Load COCO-style annotations from train/test/valid splits."""
    all_images = []
    all_anns = []
    all_categories = []
...
def load_dataset(base_path: Path, name: str):
    """Load standard COCO annotations plus any split-level stats."""
    if base_path.name == 'Chromosome20240904_NoAug_NoResize_coco':
        return load_coco_annotations(base_path, name)
    elif base_path.name == '24_chromosomes_object':
        return load_coco_annotations(base_path / 'coco', name)
    else:
        raise ValueError(f'Unknown dataset: {base_path}')


def build_dataframes():
    """Load both datasets and return DataFrames for analysis."""
    dfs = {}
    for ds_name, ds_path in DATASETS.items():
        print(f'Loading {ds_name}...')
        images, annotations, categories = load_dataset(ds_path, ds_name)
