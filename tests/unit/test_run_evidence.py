...
def test_artifact_paths_are_project_relative():
    ...
    assert any("project-relative" in error
               for error in validate(bad, Path.cwd(), False))


def prepared_import(tmp_path):
    artifacts = {}
    for name in ("annotation.json", "config.py", "checkpoint.pth", "train.log"):
        path = tmp_path / name
    ...
    connection.execute(
        ...
        ("dataset-manifest", "repository", "manifest.json", SHA,
    ...
    connection.execute(
        ...
        "VALUES (?,?,?,?,?,?,?,?,?)",
        ("D2", "Dataset 2", "data/d2", "1", 1000, 24, "{}",
    ...
    connection.execute(
        ...
        (evidence["train_run_id"], "D2", "model", 42, "config.py",
         artifacts["config.py"]["sha256"], SHA, "abcdef1",
         "independent_training_seed", "gpu", "work_dirs/model", "trained"),
    )
    connection.commit()
...
def test_import_is_transactional_and_idempotent(tmp_path):
    ...
    assert connection.execute(
        "SELECT COUNT(*) FROM controlled_result").fetchone()[0] == 6
    assert connection.execute(
        "SELECT COUNT(*) FROM eval_run_registry").fetchone()[0] == 1
    connection.close()
