...
from ldmdet.utils.box_ops import (
    bbox2roi,
    bbox_cxcywh_to_xyxy,
    bbox_xyxy_to_cxcywh,
)
...
class TestBoxOps:
    def test_xyxy_to_cxcywh(self):
        bbox = torch.tensor(
            [[100.0, 100.0, 200.0, 200.0], [50.0, 50.0, 150.0, 150.0]]
        )
        result = bbox_xyxy_to_cxcywh(bbox)
        expected = torch.tensor(
            [[150.0, 150.0, 100.0, 100.0], [100.0, 100.0, 100.0, 100.0]]
        )
    ...
    def test_roundtrip(self):
        original = torch.tensor(
            [[10.0, 20.0, 110.0, 220.0], [0.0, 0.0, 50.0, 80.0]]
        )
    ...
    def test_bbox2roi(self):
        bboxes = [
            ...
            torch.tensor([[50, 60, 70, 80], [90, 100, 110, 120]]),
        ]
