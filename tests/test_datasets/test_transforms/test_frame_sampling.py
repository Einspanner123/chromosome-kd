...
class TestFrameSample(unittest.TestCase):
    ...
    def test_base_frame_sample(self):
        ...
        assert (
            results['img'][0] == np.zeros((self.H, self.W, 3)) + key_frame_id
        ).all()
        assert (
            results['gt_bboxes'][0] == np.zeros((2, 4)) + key_frame_id
        ).all()
        assert (
            results['gt_bboxes_labels'][0] == np.zeros((2,)) + key_frame_id
        ).all()
        assert (
            results['gt_instances_id'][0] == np.zeros((2,)) + key_frame_id
        ).all()
        assert results['ori_shape'][0] == (
            self.H + key_frame_id,
            self.W + key_frame_id,
        )
    ...
    def test_uniform_ref_frame_sample(self):
        ...
        key_frame_id = self.video_infos['key_frame_id']
        assert (
            results['img'][1] == np.zeros((self.H, self.W, 3)) + key_frame_id
        ).all()
        assert (
            results['gt_bboxes'][1] == np.zeros((2, 4)) + key_frame_id
        ).all()
        assert (
            results['gt_bboxes_labels'][1] == np.zeros((2,)) + key_frame_id
        ).all()
        assert (
            results['gt_instances_id'][1] == np.zeros((2,)) + key_frame_id
        ).all()
        assert results['ori_shape'][1] == (
            self.H + key_frame_id,
            self.W + key_frame_id,
        )
