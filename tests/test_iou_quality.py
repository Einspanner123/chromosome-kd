...
def test_quality_head_shape_initialization_and_gradient():
    ...
    assert torch.allclose(
        quality.sigmoid().mean(), torch.tensor(0.9), atol=0.01
    )
...
def test_quality_only_mode_requires_enabled_quality_head():
    try:
        DiffusionDetHead(
            ...
            num_proposals=3,
            num_heads=2,
            single_head=make_single_head(False),
            roi_extractor=None,
            criterion=None,
        ...
        raise AssertionError(
            'quality-only mode accepted a missing quality head'
        )
...
def test_quality_calibration_mode_is_validated():
    try:
        DiffusionDetHead(
            ...
            num_proposals=3,
            num_heads=2,
            single_head=make_single_head(True),
            roi_extractor=None,
            criterion=None,
            quality_calibration_mode='unknown',
        )
    except ValueError as error:
...
def test_final_only_ranking_does_not_overwrite_internal_logits():
    ...
    assert torch.allclose(
        ranked.sigmoid(), internal.sigmoid() * 0.25, atol=1e-6
    )
