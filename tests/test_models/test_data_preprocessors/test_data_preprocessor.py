...
class TestDetDataPreprocessor(TestCase):
    ...
    def test_forward(self):
        ...
        batch_data_samples = processor(packed_inputs, training=True)[
            'data_samples'
        ]
        for data_samples, expected_shape, mask_pad_sum, seg_pad_sum in zip(
            ...
            self.assertEqual(
                data_samples.gt_instances.masks.masks.shape[-2:],
                expected_shape,
            )
            self.assertEqual(
                data_samples.gt_sem_seg.sem_seg.shape[-2:], expected_shape
            )
            self.assertEqual(
                data_samples.gt_instances.masks.masks.sum(), mask_pad_sum
            )
            self.assertEqual(
                data_samples.gt_sem_seg.sem_seg.sum(), seg_pad_sum
            )
...
class TestMultiBranchDataPreprocessor(TestCase):
    ...
    def test_multi_data_preprocessor(self):
        ...
        self.assertEqual(
            multi_data['inputs']['unsup_teacher'].shape, (2, 3, 608, 608)
        )
        self.assertEqual(
            multi_data['inputs']['unsup_student'].shape, (2, 3, 704, 704)
        )
