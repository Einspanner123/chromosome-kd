...
def _rand_masks(num_items, bboxes, img_w, img_h):
    ...
    for i, bbox in enumerate(bboxes):
        ...
        mask = (
            rng.rand(1, bbox[3] - bbox[1], bbox[2] - bbox[0]) > 0.3
        ).astype(np.int64)
...
class TestBoxInstHead(TestCase):
    def test_boxinst_maskhead_loss(self):
        ...
        self.assertEqual(
            loss_mask_project, 0, 'mask project loss should be zero'
        )
        self.assertEqual(
            loss_mask_pairwise, 0, 'mask pairwise loss should be zero'
        )
        ...
        self.assertGreater(
            loss_mask_project, 0, 'mask project loss should be nonzero'
        )
        self.assertGreater(
