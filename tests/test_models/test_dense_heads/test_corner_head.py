...
class TestCornerHead(TestCase):
    def test_corner_head_loss(self):
        ...
        self.assertTrue(
            empty_det_loss.item() > 0, 'det loss should be non-zero'
        )
        self.assertTrue(
        ...
        self.assertTrue(
            onegt_det_loss.item() > 0, 'det loss should be non-zero'
        )
        self.assertTrue(
        ...
        self.assertTrue(
            onegt_pull_loss.item() > 0, 'pull loss should be non-zero'
        )
        self.assertTrue(
            onegt_off_loss.item() > 0, 'off loss should be non-zero'
        )
        ...
        self.assertTrue(
            twogt_det_loss.item() > 0, 'det loss should be non-zero'
        )
        # F.relu limits push loss larger than or equal to 0.
        self.assertTrue(
            twogt_push_loss.item() >= 0, 'push loss should be non-zero'
        )
        self.assertTrue(
            twogt_pull_loss.item() > 0, 'pull loss should be non-zero'
        )
        self.assertTrue(
            twogt_off_loss.item() > 0, 'off loss should be non-zero'
        )
    ...
    def test_corner_head_encode_and_decode_heatmap(self):
        ...
        gt_bboxes = [
            torch.Tensor(
                [[10, 20, 200, 240], [40, 50, 100, 200], [10, 20, 200, 240]]
            )
        ]
        ...
        iou_matrix = bbox_overlaps(
            offset_bboxes.numpy(), offset_gtbboxes.numpy()
        )
