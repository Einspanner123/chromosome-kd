...
class TestDeformableDETR(TestCase):
    ...
    def test_deformable_detr_head_loss(self):
        ...
        configs = [
            ...
            get_detector_cfg(
                'deformable_detr/deformable-detr-refine_r50_16xb2-50e_coco.py'
            ),
            get_detector_cfg(
                'deformable_detr/deformable-detr-refine-twostage_r50_16xb2-50e_coco.py'
            ),
        ]
        ...
        for config in configs:
            ...
            for key, loss in empty_gt_losses.items():
                if 'cls' in key:
                    self.assertGreater(
                        loss.item(), 0, 'cls loss should be non-zero'
                    )
                elif 'bbox' in key:
                    self.assertEqual(
                        loss.item(),
