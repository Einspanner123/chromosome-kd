...
class TestMaskIoUHead(TestCase):
    ...
    def test_mask_iou_head_loss_and_target(self, device):
        ...
        (batch_gt_instances, batch_gt_instances_ignore, _) = (
        ...
        pos_mask_iou_pred = mask_iou_pred[
            range(mask_iou_pred.size(0)), pos_labels
        ]
    ...
    def test_mask_iou_head_predict_by_feat(self, device):
        ...
        mask_iou_preds = mask_iou_head(
            mask_feats,
            mask_preds[range(results.labels.size(0)), results.labels],
        )
