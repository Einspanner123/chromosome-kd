...
def test_sparse_prior():
    ...
    if torch.cuda.is_available():
        mlvl_points = MlvlPointGenerator(strides=[4, 10], offset=0)
        prior_indexs = (
            torch.Tensor([0, 3, 4, 5, 6, 7, 1, 2, 4, 5, 6, 9]).long().cuda()
        )
        ...
        prior_indexs = (
            torch.Tensor([4, 5, 6, 7, 0, 2, 50, 4, 5, 6, 9]).long().cuda()
        )
...
def test_standard_anchor_generator():
    ...
    assert (
        anchor_generator.num_base_priors == anchor_generator.num_base_anchors
    )
...
def test_guided_anchor():
    ...
    multi_level_valid_flags = (
        ga_retina_head.approx_anchor_generator.valid_flags(
