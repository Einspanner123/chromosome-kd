...
class TestHungarianAssigner(TestCase):
    ...
    def test_hungarian_match_assigner(self):
        ...
        self.assertEqual(
            (assign_result.gt_inds > 0).sum(), gt_instances.bboxes.size(0)
        )
        self.assertEqual(
            (assign_result.labels > -1).sum(), gt_instances.bboxes.size(0)
        )
    ...
    def test_mask_match_cost(self):
        ...
        self.assertTrue(torch.all(assign_result.gt_inds > -1))
        self.assertEqual(
            (assign_result.gt_inds > 0).sum(), gt_instances.masks.size(0)
        )
        self.assertEqual(
            (assign_result.labels > -1).sum(), gt_instances.masks.size(0)
        )
