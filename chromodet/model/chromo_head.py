...
class ChromoDetDynamicHead(DynamicDiffusionDetHead):
    ...
    def forward(self, features, init_bboxes, init_t, init_features=None):
        ...
        for head_idx, single_head in enumerate(self.head_series):
            ...
            if head_idx == len(
                    self.head_series) - 1 and self.use_topology_pairing:
                # 自注意力建模染色体间关系
        ...
        if self.deep_supervision:
            if self.use_length_prior or self.use_length_ordering:
                output['aux_outputs'] = [{
                    ...
                    'pred_boxes': b,
                    'pred_lengths': c
                } for a, b, c in zip(pred_logits[:-1], pred_bboxes[:-1],
                                     pred_lengths[:-1])]
            else:
                output['aux_outputs'] = [{
                    'pred_logits': a,
            ...
            for idx in range(batch_size):
                ...
                if self.box_renewal:  # 如果使用框更新
                    ...
                    if num_remain < self.num_proposals:
                        ...
                        select_mask = torch.randperm(
                            num_remain)[:self.num_proposals]  # 使用torch自带随机序号方法
            ...
            for idx in range(batch_size):
                ...
                if use_nms:
                    ...
                    idxs_for_nms = (
                        ...
                        if class_agnostic else labels_per_image)
