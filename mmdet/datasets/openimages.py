...
class OpenImagesDataset(BaseDetDataset):
    ...
    def _parse_label_file(self, label_file: str) -> tuple:
        ...
        classes_names = []
        with get_local_path(
            label_file, backend_args=self.backend_args
        ) as local_path, open(local_path) as f:
            reader = csv.reader(f)
            for line in reader:
    ...
    def _parse_img_level_ann(
        self, img_level_ann_file: str
    ) -> Dict[str, List[dict]]:
...
class OpenImagesChallengeDataset(OpenImagesDataset):
    ...
    def __init__(self, ann_file: str, **kwargs) -> None:
        if not ann_file.endswith('txt'):
            raise TypeError(
                'The annotation file of Open Images Challenge '
