...
def check_for_positive_overflow(
    gt_bboxes, gt_labels, text, tokenizer, max_tokens
):
    ...
    return (
        gt_bboxes[keep_box_index],
        np.array(keep_gt_labels, dtype=np.long),
        length,
    )
...
class RandomSamplingNegPos(BaseTransform):
    ...
    def od_aug(self, results):
        ...
        if len(gt_bboxes) < original_box_num:
            print(
                f'WARNING: removed {original_box_num - len(gt_bboxes)} boxes due to positive caption overflow'
            )
