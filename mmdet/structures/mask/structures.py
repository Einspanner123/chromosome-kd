...
class BitmapMasks(BaseInstanceMasks):
    ...
    def expand(self, expanded_h, expanded_w, top, left):
        ...
        if len(self.masks) == 0:
            expanded_mask = np.empty(
                (0, expanded_h, expanded_w), dtype=np.uint8
            )
        else:
        ...
        if len(self.masks) == 0:
            ...
            if masks.shape[-2:] != out_shape:
                empty_masks = np.zeros(
                    (masks.shape[0], *out_shape), dtype=masks.dtype
                )
    ...
    def shear(
        ...
        if len(self.masks) == 0:
            ...
            sheared_masks = sheared_masks.transpose((2, 0, 1)).astype(
                self.masks.dtype
            )
    ...
    def rotate(
        ...
        if len(self.masks) == 0:
            ...
            rotated_masks = rotated_masks.transpose((2, 0, 1)).astype(
                self.masks.dtype
            )
    ...
    @classmethod
    def random(
        cls, num_masks=3, height=32, width=32, dtype=np.uint8, rng=None
    ):
...
class PolygonMasks(BaseInstanceMasks):
    ...
    Example:
        ...
        >>> # demo translate
        >>> new = self.translate((16, 16), 4.0, direction='horizontal')
    ...
    def flip(self, flip_direction='horizontal'):
        ...
        if len(self.masks) == 0:
            ...
            flipped_masks = PolygonMasks(
                flipped_masks, self.height, self.width
            )
            ...
            for poly_per_obj in self.masks:
                ...
                for p in poly_per_obj:
                    ...
                    for poly in cropped:
                        ...
                        if (
                            not isinstance(poly, geometry.Polygon)
    ...
    def translate(
        ...
        Example:
            ...
            >>> new = self.translate(out_shape, 4.0, direction='horizontal')
            >>> assert np.all(new.masks[0][0][1::2] == self.masks[0][0][1::2])
            >>> assert np.all(
            ...     new.masks[0][0][0::2] == self.masks[0][0][0::2] + 4
            ... )  # noqa: E501
        """
    ...
    def shear(
        ...
        if len(self.masks) == 0:
            ...
            if direction == 'horizontal':
                ...
                shear_matrix = np.stack([[1, 0], [magnitude, 1]]).astype(
                    np.float32
                )
            for poly_per_obj in self.masks:
                sheared_poly = []
                for p in poly_per_obj:
                    ...
                    new_coords[0, :] = np.clip(
                        new_coords[0, :], 0, out_shape[1]
                    )
                    new_coords[1, :] = np.clip(
                        new_coords[1, :], 0, out_shape[0]
                    )
                    sheared_poly.append(
    ...
    def to_tensor(self, dtype, device):
        ...
        if len(self.masks) == 0:
            return torch.empty(
                (0, self.height, self.width), dtype=dtype, device=device
            )
    ...
    def random(
        ...
        def _order_vertices(verts):
            ...
            tau = np.pi * 2
            angle = (
                np.arctan2(mlat - verts.T[0], verts.T[1] - mlng) + tau
            ) % tau
            sortx = angle.argsort()
