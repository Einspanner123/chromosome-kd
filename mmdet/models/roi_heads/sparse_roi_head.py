...
class SparseRoIHead(CascadeRoIHead):
    ...
    def predict_mask(
        ...
        for img_id in range(num_imgs):
            topk_inds = topk_inds_list[img_id]
            masks_per_img = mask_results['mask_preds'][img_id].flatten(0, 1)[
                topk_inds
            ]
            masks_per_img = masks_per_img[:, None, ...].repeat(
                1, num_classes, 1, 1
            )
            ...
            for stage in range(self.num_stages):
                ...
                if self.with_mask:
                    ...
                    attn_feats = torch.cat(
                        [
                            feats[res.pos_inds]
                            for (feats, res) in zip(
                                attn_feats, sampling_results
                            )
                        ]
                    )
                    mask_results = self._mask_forward(
                        stage, x, pos_rois, attn_feats
                    )
