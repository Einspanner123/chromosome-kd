...
class VFNetHead(ATSSHead, FCOSHead):
    ...
    def __init__(
        ...
        init_cfg: MultiConfig = dict(
            ...
            std=0.01,
            override=dict(
                type='Normal', name='vfnet_cls', std=0.01, bias_prob=0.01
            ),
        ),
        **kwargs,
    ) -> None:
    ...
    def forward_single(
        ...
        if self.bbox_norm_type == 'reg_denom':
            bbox_pred = (
                scale(self.vfnet_reg(reg_feat_init)).float().exp() * reg_denom
            )
        elif self.bbox_norm_type == 'stride':
            bbox_pred = (
                scale(self.vfnet_reg(reg_feat_init)).float().exp() * stride
            )
        else:
        ...
        dcn_offset = self.star_dcn_offset(
            bbox_pred, self.gradient_mul, stride
        ).to(reg_feat.dtype)

        # refine the bbox_pred
        reg_feat = self.relu(self.vfnet_reg_refine_dconv(reg_feat, dcn_offset))
        bbox_pred_refine = (
            scale_refine(self.vfnet_reg_refine(reg_feat)).float().exp()
        )
    ...
    def loss_by_feat(
        ...
        bbox_avg_factor_ini = (
            reduce_mean(bbox_weights_ini.sum()).clamp_(min=1).item()
        )
        ...
        bbox_avg_factor_rf = (
            reduce_mean(bbox_weights_rf.sum()).clamp_(min=1).item()
        )
    ...
    def get_fcos_targets(
        ...
        labels, bbox_targets = FCOSHead.get_targets(
            self, points, batch_gt_instances
        )
        label_weights = None
        bbox_weights = None
    ...
    def transform_bbox_targets(
        ...
        for i in range(num_levels):
            bbox_target = self.bbox_coder.encode(
                mlvl_points[i], decoded_bboxes[i]
            )
