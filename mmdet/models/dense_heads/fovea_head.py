...
class FoveaHead(AnchorFreeHead):
    ...
    def _get_targets_single(
        ...
        for base_len, (
            lower_bound,
            upper_bound,
        ), stride, featmap_size, priors in zip(
            self.base_edge_list,
            self.scale_ranges,
            self.strides,
            featmap_size_list,
            ...
            bbox_targets = gt_bboxes_raw.new_ones(
                featmap_size[0], featmap_size[1], 4
            )
            # scale assignment
            hit_indices = (
    ...
    def _bbox_decode(
        ...
        y = priors[:, 1]
        x = priors[:, 0]
        x1 = (x - base_len * bbox_pred[:, 0]).clamp(
            min=0, max=max_shape[1] - 1
        )
        y1 = (y - base_len * bbox_pred[:, 1]).clamp(
            min=0, max=max_shape[0] - 1
        )
        x2 = (x + base_len * bbox_pred[:, 2]).clamp(
            min=0, max=max_shape[1] - 1
        )
        y2 = (y + base_len * bbox_pred[:, 3]).clamp(
            min=0, max=max_shape[0] - 1
        )
