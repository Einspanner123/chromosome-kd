...
class CornerHead(BaseDenseHead):
    ...
    def get_targets(
        ...
        for batch_id in range(batch_size):
            ...
            for box_id in range(len(gt_labels[batch_id])):
                ...
                gt_br_offset[batch_id, 1, bottom_idx, right_idx] = (
                    bottom_offset
                )
                ...
                if with_corner_emb:
                    corner_match.append(
                        [[top_idx, left_idx], [bottom_idx, right_idx]]
                    )
                ...
                if with_centripetal_shift:
                    gt_tl_centripetal_shift[batch_id, 0, top_idx, left_idx] = (
                        log(scale_center_x - scale_left)
                    )
                    gt_tl_centripetal_shift[batch_id, 1, top_idx, left_idx] = (
                        log(scale_center_y - scale_top)
                    )
                    gt_br_centripetal_shift[
                        batch_id, 0, bottom_idx, right_idx
                    ] = log(scale_right - scale_center_x)
                    gt_br_centripetal_shift[
                        batch_id, 1, bottom_idx, right_idx
                    ] = log(scale_bottom - scale_center_y)
    ...
    def loss_by_feat_single(
        ...
        tl_off_mask = (
            gt_tl_hmp.eq(1).sum(1).gt(0).unsqueeze(1).type_as(gt_tl_hmp)
        )
        br_off_mask = (
            gt_br_hmp.eq(1).sum(1).gt(0).unsqueeze(1).type_as(gt_br_hmp)
        )
    ...
    def _bboxes_nms(
        ...
        if 'nms_cfg' in cfg:
            warning.warn(
                'nms_cfg in test_cfg will be deprecated. '
    ...
    def _decode_heatmap(
        ...
        if with_centripetal_shift:
            ...
            ct_bboxes = torch.stack(
                (tl_ctxs, tl_ctys, br_ctxs, br_ctys), dim=3
            )
