...
class GuidedAnchorHead(AnchorHead):
    ...
    def __init__(
        ...
        assert (
            approx_anchor_generator['strides']
        ...
        self.approxs_per_octave = self.approx_anchor_generator.num_base_priors[
            0
        ]
    ...
    def _init_layers(self) -> None:
        ...
        self.conv_shape = nn.Conv2d(
            self.in_channels, self.num_base_priors * 2, 1
        )
    ...
    def get_sampled_approxs(
        ...
        for img_id, img_meta in enumerate(batch_img_metas):
            ...
            multi_level_approx_flags = (
                self.approx_anchor_generator.valid_flags(
    ...
    def ga_loc_targets(
        ...
        for img_id in range(img_per_gpu):
            ...
            for gt_id in range(gt_bboxes.size(0)):
                ...
                all_loc_weights[lvl][
                    img_id,
                    0,
                    ignore_y1 : ignore_y2 + 1,
                    ignore_x1 : ignore_x2 + 1,
                ] = 0
                all_loc_weights[lvl][
                    img_id, 0, ctr_y1 : ctr_y2 + 1, ctr_x1 : ctr_x2 + 1
                ] = 1
        ...
        loc_avg_factor = (
            sum([t.size(0) * t.size(-1) * t.size(-2) for t in all_loc_targets])
            / 200
        )
    ...
    def ga_shape_targets(
        ...
        bbox_anchors_list = images_to_levels(
            all_bbox_anchors, num_level_squares
        )
        bbox_gts_list = images_to_levels(all_bbox_gts, num_level_squares)
        bbox_weights_list = images_to_levels(
            all_bbox_weights, num_level_squares
        )
        return (
            bbox_anchors_list,
            bbox_gts_list,
    ...
    def loss_shape_single(
        ...
        inds = torch.nonzero(anchor_weights[:, 0] > 0, as_tuple=False).squeeze(
            1
        )
