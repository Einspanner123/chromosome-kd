...
class SABLRetinaHead(BaseDenseHead):
    ...
    def get_targets(
        ...
        label_weights_list = images_to_levels(
            all_label_weights, num_level_squares
        )
    ...
    def _get_targets_single(
        ...
        if unmap_outputs:
            ...
            bbox_cls_targets = unmap(
                bbox_cls_targets, num_total_anchors, inside_flags
            )
            bbox_cls_weights = unmap(
                bbox_cls_weights, num_total_anchors, inside_flags
            )
            bbox_reg_targets = unmap(
                bbox_reg_targets, num_total_anchors, inside_flags
            )
            bbox_reg_weights = unmap(
                bbox_reg_weights, num_total_anchors, inside_flags
            )
        return (
            labels,
            label_weights,
