...
class PISARetinaHead(RetinaHead):
    ...
    def loss_by_feat(
        ...
        flatten_label_weights = torch.cat(label_weights_list, dim=1).reshape(
            -1
        )
        flatten_anchors = torch.cat(all_anchor_list, dim=1).reshape(-1, 4)
        flatten_bbox_targets = torch.cat(bbox_targets_list, dim=1).reshape(
            -1, 4
        )
        flatten_bbox_weights = torch.cat(bbox_weights_list, dim=1).reshape(
            -1, 4
        )
