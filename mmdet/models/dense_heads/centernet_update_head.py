...
def _transpose(
    tensor_list: List[Tensor], num_point_list: list
) -> List[Tensor]:
...
class CenterNetUpdateHead(AnchorFreeHead):
    ...
    def loss_by_feat(
        ...
        # 5 calc reg loss
        pos_bbox_inds = torch.nonzero(bbox_targets.max(dim=1)[0] >= 0).squeeze(
            1
        )
    ...
    def add_cls_pos_inds(
        ...
        loss_thr = torch.kthvalue(
            center3x3_bbox_loss, self.more_pos_topk, dim=1
        )[0]
        ...
        cls_labels = cls_labels.view(num_gts, 1).expand(
            num_gts, total_level * K
        )[new_pos]
    ...
    def _get_center3x3_region_index_targets(
        ...
        K = 9
        dx = shapes_per_level.new_tensor([-1, 0, 1, -1, 0, 1, -1, 0, 1]).view(
            1, 1, K
        )
        dy = shapes_per_level.new_tensor([-1, -1, -1, 0, 0, 0, 1, 1, 1]).view(
            1, 1, K
        )
        ...
        for im_i in range(batch):
            ...
            expanded_strides = strides[None, :, None].expand(
            ...
            centers_discret = (
                centers_inds * expanded_strides + expanded_strides // 2
            )

            bbox_target = bbox2distance(
                centers_discret, gt_bboxes
            )  # M x N x 4
            ...
            center3x3_bbox_target = (
                bbox_target[..., None, :]
                .expand(num_gts, total_levels, K, 4)
                .clone()
            )
