...
class CenterPrior(nn.Module):
    ...
    def forward(
        ...
        for slvl_points, stride in zip(anchor_points_list, self.strides):
            ...
            center_prior = torch.exp(-distance / (2 * instance_sigma**2)).prod(
                dim=-1
            )
            ...
            if gt_inds_no_points_inside.numel():
                ...
                inside_gt_bbox_mask[:, gt_inds_no_points_inside] = (
                    torch.scatter(
                        ...
                        src=torch.ones_like(
                            topk_center_index, dtype=torch.bool
                        ),
                    )
                )
...
class AutoAssignHead(FCOSHead):
    ...
    def loss_by_feat(
        ...
        pos_avg_factor = reduce_mean(bbox_pred.new_tensor(all_num_gt)).clamp_(
            min=1
        )
        ...
        neg_avg_factor = sum(
            item.data.sum() for item in center_prior_weight_list
        )
        ...
        if all_num_gt == 0:
            ...
            dummy_center_prior_loss = (
                self.center_prior.mean.sum() * 0
                + self.center_prior.sigma.sum() * 0
            )
