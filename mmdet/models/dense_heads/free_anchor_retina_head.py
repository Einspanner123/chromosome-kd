...
class FreeAnchorRetinaHead(RetinaHead):
    ...
    def loss_by_feat(
        ...
        cls_scores = [
            cls.permute(0, 2, 3, 1).reshape(
                cls.size(0), -1, self.cls_out_channels
            )
        ...
        negative_loss = self.negative_bag_loss(
            cls_probs, box_probs
        ).sum() / max(1, num_pos * self.pre_anchor_topk)
    ...
    def positive_loss_single(
        ...
        positive_loss = self.positive_bag_loss(
            matched_cls_prob, matched_box_prob
        )
