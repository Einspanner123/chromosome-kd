...
class Mask2FormerTrackHead(MaskFormerHead):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.level_embed = nn.Embedding(
            self.num_transformer_feat_level, feat_channels
        )
    ...
    def preprocess_gt(self, batch_gt_instances: InstanceList) -> InstanceList:
        ...
        for batch_idx in range(batch_size):
            ...
            assert len(pair_gt_insatences) > 1, (
            ...
            for frame_id in range(self.num_frames):
                ...
                for i, id in enumerate(ins_ids):
                    gt_masks_per_video[map_ins_id[id], frame_id, :, :] = (
                        cur_frame_gts.masks[i]
                    )
                    gt_ids_per_video[map_ins_id[id], frame_id] = (
                        cur_frame_gts.instances_ids[i]
                    )
                    gt_labels_per_video[map_ins_id[id]] = cur_frame_gts.labels[
                        i
                    ]
    ...
    def _get_targets_single(
        ...
        point_coords = torch.rand(
            (1, self.num_points, 2), device=cls_score.device
        )
    ...
    def forward(
        ...
        for i in range(self.num_transformer_feat_level):
            ...
            decoder_positional_encoding = (
                decoder_positional_encoding.flatten(3)
                .permute(0, 1, 3, 2)
                .flatten(1, 2)
            )
