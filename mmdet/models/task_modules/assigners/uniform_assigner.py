...
class UniformAssigner(BaseAssigner):
    ...
    def assign(
        ...
        indexes = (
            torch.cat((index, index1), dim=1).reshape(-1).to(bbox_pred.device)
        )
        ...
        pos_gt_index = torch.arange(
            0, C1.size(1), device=bbox_pred.device
        ...
        assign_result.set_extra_property(
            'pos_predicted_boxes', bbox_pred[indexes]
        )
        assign_result.set_extra_property(
            'target_boxes', gt_bboxes[pos_gt_index]
        )
        return assign_result
