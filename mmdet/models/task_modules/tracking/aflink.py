...
class TemporalBlock(BaseModule):
    ...
    def __init__(
        self, in_channel: int, out_channel: int, kernel_size: tuple = (7, 1)
    ):
        super().__init__()
    ...
    def forward(self, pred_tracks: np.ndarray) -> np.ndarray:
        ...
        for i, id_i in enumerate(track_ids):
            for j, id_j in enumerate(track_ids):
                ...
                # numpy to torch
                track_i = torch.tensor(track_i, dtype=torch.float).to(
                    self.device
                )
                track_j = torch.tensor(track_j, dtype=torch.float).to(
                    self.device
                )
                ...
                confidence = (
                    self.model(track_i, track_j).detach().cpu().numpy()
                )
