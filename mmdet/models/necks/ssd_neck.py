...
class SSDNeck(BaseModule):
    ...
    def __init__(
        ...
        second_conv = (
            DepthwiseSeparableConvModule if use_depthwise else ConvModule
        )
        ...
        for i, (out_channel, stride, padding) in enumerate(
            ...
            kernel_size = (
                last_kernel_size if i == len(extra_layer_channels) - 1 else 3
            )
...
class L2Norm(nn.Module):
    ...
    def forward(self, x):
        ...
        return (
            self.weight[None, :, None, None].float().expand_as(x_float)
            * x_float
            / norm
        ).type_as(x)
