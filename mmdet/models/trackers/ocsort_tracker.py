...
class OCSORTTracker(SORTTracker):
    ...
    def vel_direction_batch(
        self, bboxes1: torch.Tensor, bboxes2: torch.Tensor
    ):
        ...
        speed = torch.cat(
            (speed_diff_y[..., None], speed_diff_x[..., None]), dim=-1
        )
        norm = torch.sqrt((speed[:, :, 0]) ** 2 + (speed[:, :, 1]) ** 2) + 1e-6
        speed[:, :, 0] /= norm
        speed[:, :, 1] /= norm
    ...
    def ocm_assign_ids(
        ...
        if len(ids) > 0 and len(det_bboxes) > 0:
            track_velocities = torch.stack(
                [self.tracks[id].velocity for id in ids]
            ).to(det_bboxes.device)
            ...
            vel_to_match = self.vel_direction_batch(
                k_step_observations, det_bboxes
            )
    ...
    def track(self, data_sample: DetDataSample, **kwargs) -> InstanceData:
        ...
        if self.empty or bboxes.size(0) == 0:
            ...
            ids = torch.arange(
                self.num_tracks, self.num_tracks + num_new_tracks
            ).to(labels)
            ...
            match_det_ids = torch.cat(
                (match_det_ids, unmatch_det_ids[valid]), dim=0
            )
            assert (match_det_ids > -1).all()
