...
class TrackDataPreprocessor(DetDataPreprocessor):
        ...
        **kwargs,
        ...
        if mean is not None and not self.use_det_processor:
            ...
            self.register_buffer(
                'mean', torch.tensor(mean).view(1, -1, 1, 1), False
            )
            self.register_buffer(
                'std', torch.tensor(std).view(1, -1, 1, 1), False
            )
    ...
    def forward(self, data: dict, training: bool = False) -> Dict:
        ...
        if self.use_det_processor and training:
            ...
            if self._enable_normalize:
                imgs = [(_img - self.mean) / self.std for _img in imgs]
            inputs = stack_track_batch(
                imgs, self.pad_size_divisor, self.pad_value
            )
        ...
        if training and self.batch_augments is not None:
            for batch_aug in self.batch_augments:
                if self.use_det_processor and training:
                    ...
                    assert inputs.size(1) == 1 and len(data_samples[0]) == 1, (
                        'Only support the number of sequence images equals to 1 when using batch augment.'
                    )
                    det_data_samples = [
                        track_data_sample[0]
    ...
    def pad_track_gt_masks(
        self, data_samples: Sequence[TrackDataSample]
    ) -> None:
...
def stack_track_batch(
    ...
    assert tensors[0].ndim == 4, (
        f'Expected tensor dimension to be 4, but got {tensors[0].ndim}'
    )
