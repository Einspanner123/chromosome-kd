...
class SoftTeacher(SemiBaseDetector):
    ...
    def loss_by_pseudo_instances(
        ...
        return rename_loss_dict(
            'unsup_', reweight_loss_dict(losses, unsup_weight)
        )
    ...
    def rcnn_cls_loss_by_pseudo_instances(
        ...
        with torch.no_grad():
            ...
            neg_inds = (
                cls_reg_targets[0]
                == self.student.roi_head.bbox_head.num_classes
            )
    ...
    def rcnn_reg_loss_by_pseudo_instances(
        ...
        roi_losses = self.student.roi_head.loss(
            x, rpn_results_list, reg_data_samples
        )
    ...
    def compute_uncertainty_with_aug(
        ...
        reg_channel = (
            max([results.bboxes.shape[-1] for results in results_list]) // 4
        )
        bboxes = [
        ...
        box_shape = [
            (bbox[:, 2:4] - bbox[:, :2]).clamp(min=1.0) for bbox in bboxes
        ]
        box_unc = [
            torch.mean(
    ...
    def aug_box(batch_data_samples, times, frac):
        ...
        def _aug_single(box):
            ...
            new_box = (
                box.clone()[None, ...].expand(times, box.shape[0], -1) + offset
            )
            return new_box
