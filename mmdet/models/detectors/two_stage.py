...
class TwoStageDetector(BaseDetector):
    ...
    def _load_from_state_dict(
        ...
        bbox_head_keys = [
            k for k in state_dict if k.startswith(bbox_head_prefix)
        ]
        rpn_head_prefix = prefix + '.rpn_head' if prefix else 'rpn_head'
        rpn_head_keys = [
            k for k in state_dict if k.startswith(rpn_head_prefix)
        ]
        if len(bbox_head_keys) != 0 and len(rpn_head_keys) == 0:
            for bbox_head_key in bbox_head_keys:
                rpn_head_key = (
                    rpn_head_prefix + bbox_head_key[len(bbox_head_prefix) :]
                )
    ...
    def _forward(
        ...
        roi_outs = self.roi_head.forward(
