...
def get_best_epoch_or_iter(exp_dir):
    best_epoch_iter_full_path = sorted(
        glob.glob(osp.join(exp_dir, 'best_*.pth'))
    )[-1]
    best_epoch_or_iter_model_path = best_epoch_iter_full_path.split('/')[-1]
    best_epoch_or_iter = best_epoch_or_iter_model_path.split('_')[-1].split(
        '.'
    )[0]
...
def get_final_results(
    log_json_path, epoch_or_iter, results_lut, by_epoch=True
):
    result_dict = dict()
    last_val_line = None
...
def main():
    ...
    for used_config in used_configs:
        ...
        # get the latest logs
        log_json_path = sorted(glob.glob(osp.join(exp_dir, '*.log.json')))[-1]
        log_txt_path = sorted(glob.glob(osp.join(exp_dir, '*.log')))[-1]
        ...
        model_performance = get_final_results(
            log_json_path, final_epoch_or_iter, results_lut, by_epoch
        )
    ...
    for model in model_infos:
        ...
        shutil.copy(
            config_path, osp.join(model_publish_dir, target_config_path)
        )
