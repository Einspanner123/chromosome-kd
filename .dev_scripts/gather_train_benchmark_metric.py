...
if __name__ == '__main__':
    ...
    if args.excel:
        assert args.ncol, (
            'Please specify "--excel" and "--ncol" at the same time'
        )
        if xlrd is None:
            raise RuntimeError(
    ...
    with open(args.txt_path) as f:
        ...
        for i, config in enumerate(model_cfgs):
            ...
            if osp.exists(result_path):
                ...
                if osp.exists(osp.join(result_path, ckpt_path)):
                    log_json_path = sorted(
                        glob.glob(osp.join(result_path, '*.log.json'))
                    )[-1]
                    ...
                    if args.excel:
                        if 'AR@1000' in model_performance:
                            metrics = (
                                f'{model_performance["AR@1000"]}(AR@1000)'
                            )
        ...
        if metrics_out:
            ...
            mmcv.dump(
                result_dict, osp.join(metrics_out, 'model_metric_info.json')
            )
