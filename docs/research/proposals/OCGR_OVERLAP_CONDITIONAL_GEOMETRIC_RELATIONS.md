...
| OCGR seed789 | 379150778 | 0.744 | 39 | epoch69 正常早停 |
...
验证脚本输出 `FIXED_VALIDATION_REPRODUCIBLE`；全部逐类别数值也相同，无大目标样本类别的 `NaN` 按缺失值等价处理。该修复只作用于新启动的训练。实际 seed 379150778 的旧进程于 epoch69 正常早停，最终验证 mAP 0.715，训练最佳仍为 epoch39 的 0.744；它在启动时已加载旧入口，其历史轨迹仍按旧随机验证协议解释。最佳 checkpoint SHA256 为 `afbe17ff9c8377d7e001d829e9515c12571f2658aae2db29b14714bbe4b82b04`。该 checkpoint 的 workstation A5000 固定复评因两台服务器无 SSH 互信且本机中转链路过慢而暂未执行，不能以 ross A6000 结果替代。
