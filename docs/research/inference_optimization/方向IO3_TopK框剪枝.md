...
### 训练结果
K=100 训练，EarlyStop 在 epoch 31，best mAP=0.823（Δ=-0.035，超出容差）。

### 推理评估（在 SOTA checkpoint a3_full_sota 上直接评估，不训练）

| K | 推理时间 | 加速比 | mAP | Δ vs 基准(0.858) |
|:---:|:---:|:---:|:---:|:---:|
| 500 (baseline) | 158.7ms | 1.00x | 0.858 | — |
| 300 | 118.8ms | 1.34x | 0.857 | -0.001 ✅ |
| 200 | 99.8ms | 1.59x | 0.856 | -0.002 ✅ |
| 100 | 79.2ms | 2.00x | 0.823（训练） | -0.035 ❌ |

### 关键发现
- **剪枝是推理时操作，无需重训**。在 SOTA checkpoint 上启用剪枝即可获得加速。
- **K=300**: 最优性价比，mAP 几乎无损（Δ=-0.001），加速 34%。
- **K=200**: 可接受，mAP 降 0.002，加速 59%。

### 验证链接
- [K=200 推理评估 (SwanLab)](https://swanlab.cn/@einspanner/ldmdet-inference/runs/f7go9wdv)
- [K=300 推理评估 (SwanLab)](https://swanlab.cn/@einspanner/ldmdet-inference/runs/dj8hxe67)
- 训练实验因 SSL 错误未上传成功。

### 最终推荐
**推荐 K=300 作为默认推理配置**：mAP Δ=-0.001，加速 34%，精度几乎无损。
