...
### 实验设计
- **状态**: 理论分析, 未进行实际实现和训练
- **依据**: 基于 IO1 实验数据 (x0_pred 步间变化 40%) 进行理论推演
...
| Best mAP | — (未实现) |
| Δ vs 基准 (0.858) | — (未实现) |
| 推理时间 | 与 baseline 相同 (未实现, 仅理论分析) |
| 理论验证通过率 | 接近 0% |
| 结论 | **证伪** — 理论存在根本缺陷 |

### 数据证据

- **投机解码的核心假设** 是"自回归模型中连续 token 的分布差异小", 但 RF 是确定性 ODE, **不满足该假设**
- **Diffusion ODE 的每一步都是确定性演化**, 不是概率采样, 没有 "draft" 和 "verify" 的区分空间
- **x0_pred 步间变化高达 40%** (来自 IO1 实验数据), 草稿模型预测的 x0 与真实 x0 差异极大, 验证通过率接近 0%
- 相邻步 x0 相对变化 mean=0.408~0.424, 任何 draft 预测都无法通过 verify

### 结论与推荐

- **投机解码不适用于扩散 ODE 推理**, 该方向理论存在根本缺陷
- **不建议继续探索**
