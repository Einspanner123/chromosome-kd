...
> ⚠️ **暂时废弃**：以下实验证据覆盖度章节中的实验数值（如 RF 0.733 vs baseline 0.725、Hard OT 0.735 vs 0.751、SOTA mAP=0.753、4-seed mean≈0.746±0.008 等）均基于旧数据集 Chromosome20240904（mAP≈0.72-0.75），24obj 数据集上的结论已更新（mAP 量级 0.77-0.87）。24obj 已有对比模型 DINO R50 0.868、LDMDet A4 0.863 等。下表的实验证据与缺口分析不可直接引用。
