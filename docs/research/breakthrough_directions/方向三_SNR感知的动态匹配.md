...
- [x] E3.0 vs E3.2：训练早期 loss 曲线更平滑，最终 mAP 提升 0.5-1% → **未通过，见 §7**
- [ ] 匹配稳定性：高 $t$ 时匹配 IoU 方差降低 > 30%

---

## 7. 实验记录 (E3.2: SNR 感知匹配 + 损失加权)

> **实验时间**：2026-06-23 ~ 2026-06-24
> **配置**：`experiments/configs/ldmdet/snr_matching.py`，继承 `rf_heun_adaln`，`SNRAwareMatcher` (logistic) + `snr_weighted_loss=True`
> **实验目录**：`work_dirs/snr_matching/`
> **SwanLab**：[snr_matching](https://swanlab.cn/@einspanner/ldmdet-ablation/runs/lqb79n2k) | [项目总览](https://swanlab.cn/@einspanner/ldmdet-ablation)

### 7.1 实现文件

| 文件 | 作用 |
|------|------|
| `ldmdet/criterion/snr_weight.py` → `get_snr_weight()` | SNR 权重计算，支持 logistic / exponential / none 三种模式 |
| `ldmdet/criterion/snr_aware_matcher.py` → `SNRAwareMatcher` | 注册为 `SNRAwareMatcher`，按 $w(t)$ 加权匹配代价，高噪声时 $k_{\max}$ 缩小 |
| `ldmdet/core/head.py` L279-321 | 集成：`loss()` 中按 $w(t)$ 加权深度监督损失 |
| `experiments/configs/ldmdet/snr_matching.py` | 实验配置，`snr_mode='logistic', snr_beta=3.0, snr_w_min=0.1, k_max_scale=True, cost_threshold=50.0` |

**参数说明**：
- `snr_mode='logistic'`：$w(t) = (1-t)^2 / ((1-t)^2 + t^2)$，$w(0)=1, w(0.5)=0.5, w(1)=0$
- `snr_w_min=0.1`：权重下界，避免高噪声时完全抑制训练信号
- `k_max_scale=True`：dynamic_k 上界按 $w(t)$ 缩放，高噪声时少分配正样本
- `cost_threshold=50.0`：高噪声（$w < 0.1$）时代价加 50，软抑制匹配
- `snr_weighted_loss=True`：损失与匹配代价加权一致

### 7.2 训练曲线

训练至 epoch 150（完整训练），best checkpoint 在 epoch 128。

| 阶段 | Epoch | mAP | AP50 | AP75 | APs | APm | APl |
|------|-------|-----|------|------|-----|------|-----|
| 快速上升 | 16 | 0.642 | 0.889 | 0.764 | 0.366 | 0.644 | 0.413 |
| 中期 | 50 | 0.685 | 0.934 | 0.810 | 0.446 | 0.686 | 0.531 |
| 中后期 | 90 | 0.710 | 0.939 | 0.824 | 0.480 | 0.705 | 0.627 |
| **best** | **128** | **0.739** | **0.943** | **0.833** | **0.521** | **0.733** | 0.601 |
| 近期 | 130 | 0.730 | 0.943 | 0.828 | 0.508 | 0.723 | 0.632 |
| 最新 | 150 | 0.716 | 0.943 | 0.821 | 0.488 | 0.712 | 0.637 |

**训练损失**：初始 12.98 → 最小 0.88 → 最后 1.06（初始损失远低于方向一/二的 ~40，可能从预训练 checkpoint 加载或 batch size 不同）。
...
| SNR Matching E3.2 | 0.739 | 0.943 | 0.833 | 0.521 | 0.733 | 0.601 | **-0.014 vs SOTA** |

### 7.4 现象分析

1. **mAP 下降 1.4 个百分点**：0.739 vs SOTA 0.753，验收标准"mAP 提升 0.5-1%"未通过，方向相反。

2. **AP50 持平但 AP75 下降**：AP50=0.943 与 SOTA 完全持平，但 AP75=0.833 vs 0.844 下降 1.1 个百分点。说明 SNR 加权对粗匹配（IoU>0.5）无影响，但损害了精细匹配（IoU>0.75）。高噪声时抑制匹配可能导致部分精细正样本被漏分配。

3. **APs 持平**：APs=0.521 与 SOTA 完全一致，SNR 加权未带来小目标改善，也未恶化。小目标在高噪声时本就难以匹配，SNR 加权的保守策略与基线行为接近。

4. **APl 显著下降**：APl=0.601 vs 0.642，下降 4.1 个百分点，是所有指标中退步最大的。大目标在高噪声时 IoU 仍可能较高（框大，IoU 容易高），SNR 加权错误地抑制了这些本应匹配的大目标 proposal。

5. **训练曲线波动大**：epoch 46=0.666 → epoch 61=0.683 → epoch 76=0.681 → epoch 91=0.715，中后期波动幅度达 5 个百分点。SNR 加权引入了额外的训练不稳定性，与文档预期"训练更稳定"相反。

6. **过拟合**：epoch 128 best 0.739 后持续下降至 0.716，降幅 2.3 个百分点，与方向一/二类似的过拟合模式。

### 7.5 根因分析

1. **SNR 加权与 RF shifted schedule 不匹配**：Rectified Flow 使用 shifted 时间采样（偏向低 $t$），训练时大部分 batch 的 $t$ 已经较低（$w(t) \approx 1$），SNR 加权几乎不生效。少数高 $t$ 的 batch 被强抑制，引入训练噪声而非稳定性。

2. **大目标匹配受损**：大目标的 IoU 在高噪声时仍可能较高（框大，即使偏移 IoU 也 > 0.5），SNR 加权错误地抑制了这些有效匹配。这解释了 APl 下降 4.1 个百分点。

3. **匹配与损失的双重加权**：同时加权匹配代价和损失，可能导致高噪声 batch 的梯度信号过弱，训练效率下降。文档 §2.3 提到的"一致性"在理论上合理，但实践中过度抑制。

### 7.6 结论

**方向三（SNR 感知动态匹配）实验失败**。best mAP=0.739，低于 SOTA 0.753 达 1.4 个百分点，验收未通过。SNR 加权在 RF shifted schedule 下效果有限，且对大目标匹配有负面影响。训练波动增大，与预期"更稳定"相反。

**状态**：⏸ 暂停。若继续，建议：
1. 仅启用匹配器加权（`snr_weighted_loss=False`），验证匹配改善的独立价值
2. 调整 `snr_w_min` 至 0.3-0.5，减少高噪声抑制强度
3. 对大目标关闭 `k_max_scale`，避免大目标匹配被错误抑制
