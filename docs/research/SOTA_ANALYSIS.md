...
> ⚠️ **暂时废弃**：以下全部排名表与模块对比结论基于旧数据集 Chromosome20240904（mAP≈0.72-0.75，排名表 mAP 0.746-0.752），24obj 数据集上的结论已更新（mAP 量级 0.77-0.87）。24obj 对比模型：RTMDet-L 0.869、DINO R50 0.868、+DPM-Solver++ (LDMDet) 0.863、Cascade R-CNN R50 0.854、YOLOX-S 0.803、DiffusionDet 0.787。本文档的模块级理论洞察（多样性管理、路径直线性等）可作为机制参考，但所有 mAP 数值不可引用。
