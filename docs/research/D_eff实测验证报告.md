...
> ⚠️ **暂时废弃**：以下 D_eff 实测数据（§2 实测方法、§3 实测结果、§4 交叉验证、§10 SOTA 代码备份交叉确认）基于旧数据集 Chromosome20240904 的 checkpoint（`reproduce_0751_stochot_eps5_v2`，mAP≈0.753）及其验证集（M≈46.6 张/图），24obj 数据集上的 D_eff 实测结论待验证。
>
> 注：§5 误差分析、§6 argmax vs sample 的 ε 依赖性机制分析、§7 对论文图表的影响判定为 OT 机制层面，与数据集无关，保留有效；但具体 D_eff 数值（10–42 范围）依赖旧数据集的 M（GT 数/图），24obj 数据集上需重测。
