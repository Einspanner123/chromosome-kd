...
> 📋 **命名约定**: 本文档使用论文正式名称 (Dataset 1 / Dataset 2 / RF+Heun / +AdaLN-Zero / +Stoch. Coupling / +DPM-Solver++ / Top-K)。内部实验代号 (24obj / A0-A4 / StochOT) 仅保留在文件路径、配置名和 SwanLab run_id 中以兼容工程实现。
>
...
3. Cascade R-CNN 路径双拼接 / YOLOX-S EMAHook / RF+Heun checkpoint 错误等修复
...
- 审计任务分 4 类:
  ...
  - **任务 D**: 5 项关键叙事一致性 (+DPM-Solver++ 唯一显著下降 / Top-K test 增强 / K=100 robust / DiffusionDet 0.787→0.803 / RTMDet-L=+DPM-Solver++ val)
...
| 编号 | 问题 | 位置 | 修复方式 | 修复状态 |
|------|------|------|----------|----------|
| P1-1 | 论文 Table 10 caption 未标 "val" (仅 "seed 42"), 且 +DPM-Solver++ 在 Table 5 (0.859) vs Table 10 (0.863) 差异未解释 | 论文 L381 | caption 加 "Dataset 2 验证集, seed 42 best checkpoint; mAP 与 FPS 测量使用同一 checkpoint, 3-seed 均值见 Table 5, 测试集评估见 §4.5.4" | ✅ 已修复 |
| P1-2 | 论文 Table 5/6 caption 用 "独立推理" 而非 "val set" | 论文 L236, L257 | caption 改为 "Dataset 2 验证集; DDPM baseline/RF+Heun/+AdaLN-Zero/比较方法为 seed 42 best checkpoint, +Stoch. Coupling 为 3-seed 均值" | ✅ 已修复 |
...
| P2-3 | 命名约定不一致: 论文与 CATALOG 对同一 checkpoint 使用不同内部代号 (如 +DPM-Solver++ 在论文原称 "A3 DPM++" 但在 CATALOG 原称 "A4"; +Stoch. Coupling 在论文原称 "A2 Stoch. Coup." 但在 CATALOG 原称 "A3 StochOT eps5") | 论文 vs CATALOG | 全文命名统一后已消除: 论文与 CATALOG 均改用累积消融正式名称 (DDPM baseline / RF+Heun / +AdaLN-Zero / +Stoch. Coupling / +DPM-Solver++), 不再使用 A0-A4 代号 | ✅ 已修复 (命名统一) |
...
|------|:---------:|:--------:|:----------:|---------------|------|
| RF+Heun | 0.856 | 0.857 | +0.001 | a1_rf_heun.log (L1606) | 稳定 |
| +Stoch. Coupling | 0.858 | 0.858 | 0.000 | a2_stochot_heun.log (L1613) | 稳定 |
| **+DPM-Solver++** | **0.863** | **0.859** | **−0.004** | a3_dpm_pp.log (L1613) | 唯一显著下降; test 0.859 = val 3-seed mean 0.859 |
| +DPM-Solver++ +Top-K K=300 | 0.861 | 0.860 | −0.001 | a3_io3_k300.log (L1616) | 稳定; test 上反超 +DPM-Solver++ |
| +DPM-Solver++ +Top-K K=200 | 0.860 | 0.859 | −0.001 | a3_io3_k200.log (L1616) | 稳定; test 上与 +DPM-Solver++ 持平 |
| +DPM-Solver++ +Top-K K=100 | 0.850 | 0.847 | −0.003 | a3_io3_k100.log (L1616) | 稳定; K=100 有害结论 robust |
...
1. ✅ **+DPM-Solver++ 唯一 val→test 显著下降 (Δ=−0.004)**: 所有文档一致
2. ✅ **Top-K K=200 在 test set 上无 mAP 损失**: 论文 §4.5.4 已显式提及 "K=200 与 +DPM-Solver++ 完全持平 (Δ=0.000, 验证集为 −0.003)"
...
5. ✅ **RTMDet-L mAP=0.863 (val) = +DPM-Solver++ val seed42 best 0.863**: 所有文档一致; 论文 Table 6 用 +DPM-Solver++ 的 3-seed mean (0.859) 进行 SOTA 对比
