...
OT 配对在低维 (d=4) 检测空间中将噪声空间划分为 Voronoi 单元, 使耦合分配成为噪声的确定性函数——耦合多样性坍缩至零 (理论分析, 命题 1-3)。标准数据增强提供的 GT 多样性在实践中补偿了这一坍缩效应, 使三种耦合策略 (Hard OT / Random / StochOT) 在精度上无显著差异 (Δ≤0.001, ns)。Stochastic Coupling 从 Sinkhorn transport 矩阵采样分配 (而非 argmax), 在 hard OT 与随机耦合之间插值, 其独立价值体现在训练稳定性 (4.6× epoch std 改善, 30/30 vs 13/30 best-1% epoch)。
...
- **小训练集理论风险**: Dataset 1 (1540 张) 上 OT 坍缩理论上界 $\Delta H \ge 0.999\log K$ 远高于 Dataset 2, 但标准数据增强提供的 GT 多样性足以补偿 Hard OT 确定性配对损失, 使坍缩在标准训练条件下不可观测 (三种耦合策略 Δ≤0.001, ns); StochOT 在小训练集上的独立价值为训练稳定性 (4.6× 平滑), 而非精度提升

### 数据集规模依赖性

| 数据集 | 规模 | Stoch. vs Random mAP Δ | 显著性 | 平滑性增益 |
|--------|------|------------------|--------|------------|
| Dataset 1 | 1540 张 | +0.001 (3-seed) | ns (3-seed 均值差) | 4.6× epoch std |
| Dataset 2 | 5000 张 | +0.0001 | p=0.80 (n=500, ns) | 4.6× epoch std |

- 标准增强下三种耦合策略 (Hard OT / Random / StochOT) 在两数据集上均无显著精度差异 (Δ≤0.001, ns), OT 坍缩效应被标准数据增强提供的 GT 多样性补偿
- StochOT 的独立价值体现在训练稳定性: Last-30 std 0.006 → 0.0013 (4.6× 改善), 30/30 vs 13/30 epoch 处于 best 1% 内
- 理论分析 (熵坍缩上界 $\Delta H \ge 0.999\log K$) 仍成立, 标准增强仅在实践中补偿了坍缩效应, 不否定理论风险的存在
...
#### 实验证明目的: Dataset 1 耦合消融 (低数据, 标准增强下无坍缩)

- Hard OT 3 seeds (Dataset 1, 标准增强)
  -- 结果: mAP=0.748 ± 0.001 (val, 3-seed: seed42=0.747@ep65, seed123=0.747@ep67, seed789=0.749@ep101); **test: 0.737 ± 0.002 (3-seed: 0.738/0.735/0.739, 补跑 2026-08-04, 见 §十三.1)**
  -- Δ(Hard OT−Random)=+0.002 (val, ns) / +0.000 (test, ns) — 标准增强下无 OT 坍缩
  -- 目录: `work_dirs/multi_seed_aug/hard_ot/seed_{42,123,789}/`
  -- SwanLab: 见下文 Random/Stoch 对照

- Random Coupling 3 seeds (Dataset 1, 标准增强)
  -- 结果: mAP=0.746 ± 0.001 (val, 3-seed); **test: 0.737±0.002 (3-seed, 见 §十三.1)**
  -- seed42=0.7450 @ ep102, seed123=0.7470 @ ep101, seed789=0.7470 @ ep75
  -- 数据源: work_dirs/multi_seed_aug/rf_heun_adaln/seed_{42,123,789}/ (find_best_map.py 核查 2026-08-03)
  -- SwanLab: 见 Dataset 2 同名实验

- Stochastic Coupling ε=5, 3 seeds (Dataset 1, 标准增强)
  -- 结果: mAP=0.747 ± 0.002 (val, 3-seed; **test: 0.738±0.004, 3-seed, 见 §十三.1**)
  -- seed42=0.7460 @ ep60, seed123=0.7460 @ ep57, seed789=0.7490 @ ep69
  -- test: seed42=0.736, seed123=0.735, seed789=0.742
  -- 数据源: work_dirs/multi_seed/stochot_eps5_old/seed_{42,123,789}/ (find_best_map.py 核查 2026-08-03)
  -- Hard OT vs Random: Δ=+0.001 (val, ns) — 标准增强下三种耦合策略无显著差异
  -- Stoch vs Random: Δ=+0.001 (val, ns) — 标准增强下 StochOT 无 measurable 增益

#### 实验证明目的: Dataset 2 耦合消融 (大数据, 标准增强下无坍缩, 平滑性显著)

- Random Coupling 3 seeds (Dataset 2, project=ldmdet-ablation)
  -- 平均: mAP=0.860 ± 0.001 (val, 3-seed); **test: 0.860±0.001 (3-seed, 补跑 2026-08-03, 见 §十三.1 D2 3-seed test 表)**
  -- 数据源: work_dirs/24obj_ablation/random/seed_{42,123,789}/ (find_best_map.py 核查 2026-08-03)
  -- seed42: mAP=0.859 (val, best @ 59; test=0.860 补跑 2026-08-03)
...
- Hard OT 3 seeds (Dataset 2, 标准增强, workstation A5000)
  -- 结果: mAP=0.861 ± 0.001 (val, 3-seed: seed42=0.860@ep89, seed123=0.861@ep59, seed789=0.861@ep98); **test: 0.862 ± 0.001 (3-seed: 0.863/0.862/0.861, 补跑 2026-08-04, 见 §十三.1)**
...
#### 实验证明目的: Dataset 1 per-class AP 对比 (标准增强, 3-seed)

- Stochastic Coupling vs Random Coupling per-class AP 对比 (3-seed, IoU=0.5:0.95, 标准增强)
  -- 标准增强下整体 mAP Δ(Stoch−Random)=+0.001 (ns), 三种耦合策略在 per-class 层面无显著差异
  -- StochOT 的主要价值体现在训练稳定性 (4.6× 平滑, 见上节) 而非精度提升
  -- 数据源: work_dirs/multi_seed_aug/rf_heun_adaln/ (Random) vs work_dirs/multi_seed/stochot_eps5_old/ (StochOT)

---

### Dataset 1 早期验证 (OT 坍缩理论佐证)

> 以下 Dataset 1 实验在标准增强下佐证 §二 OT Diversity Collapse 理论。标准增强下 Hard OT=0.748 vs Random=0.746 (Δ=+0.002, ns), 三种耦合策略无显著差异——标准数据增强提供的 GT 多样性足以补偿 Hard OT 的确定性配对损失, 使低维坍缩在标准训练条件下不可观测。Dataset 2 Hard OT 3-seed 已完成 (workstation, 2026-08-04): val 0.861±0.001, test 0.862±0.001, Δ(Hard−Random)=+0.001 (ns)。
### Hard OT Coupling

- Hard OT Coupling (Dataset 1, 标准增强, 3 seeds)
  -- 结果: mAP=0.748 ± 0.001 (val, 3-seed: seed42=0.747@ep65, seed123=0.747@ep67, seed789=0.749@ep101) [+0.002 vs 0.746 baseline (val), ns, **不显示 OT 坍缩**]; test: 0.737 ± 0.002 (3-seed: 0.738/0.735/0.739)
  -- 本地: work_dirs/multi_seed_aug/hard_ot/seed_{42,123,789}/
  -- SwanLab (project=ldmdet-ablation):
...
> ⚠ **数据 superseded (2026-08-03)**: 原 per-image Wilcoxon 检验 (Stoch−Rand +0.0308, p<10⁻¹²⁰) 基于 StochOT (标准增强) vs Random (简单增强) 的混杂对比, 非耦合策略单独效应。统一标准增强后, 3-seed aggregate 均值差 Δ(Stoch−Random)=+0.001 (ns), 原始显著性消失。论文 Table 9 需在标准增强下重新运行 per-image Wilcoxon, 或改用 3-seed aggregate 对比。

- 标准增强 3-seed aggregate 对比 (Dataset 1, val + test)

  | 比较 | Val Δ mAP | Test Δ mAP | 显著性 | 备注 |
  |------|----------|-----------|--------|------|
  | Stoch−Random | +0.001 | +0.001 | ns | 3-seed 均值差 |
  | Hard−Random | +0.002 | +0.000 | ns | 3-seed 均值差 |
  | Stoch−Hard | +0.000 | +0.001 | ns | — |

  -- 结论: 标准增强下三种耦合策略无显著精度差异 (Δ≤0.001, ns), OT 坍缩效应被标准数据增强提供的 GT 多样性补偿; StochOT 的独立价值为训练稳定性 (4.6× 平滑, 见 §二 Table 7)
  -- 数据源: §二 实验列表 + §十三.1 OT Collapse 耦合消融 (标准增强) test 表
...
- Dataset 1 DPM++ 3-seed vs Heun 对照 (2026-07-30 补全)
  ...
  -- Dataset 1 DPM++ 3-seed (训练评估, val):
     ...
     - seed123: 0.748 @ ep85 (val, early stop @ ep115, workstation A5000; **test=0.738 补跑 2026-08-03**)
     - seed789: 0.746 @ ep72 (val, 2026-07-31 重训, 旧 run 0.724@ep22 异常已消除; 早停@ep102, patience=30 触发, best 0.746 稳定 30 epoch 无刷新; **test=0.735 补跑 2026-08-03**)
     - **3-seed mean = 0.747±0.001 (val) / 0.737±0.002 (test, 补跑 2026-08-03)** (三 seed 一致性好, 无异常值; test Δ(DPM++−Heun)=0.000, 与 val Δ=+0.001 一致: DPM++ 在 D1 上与 Heun 持平)
...
- **test: 0.738±0.004 (3-seed, 补跑 2026-08-03, 见 §十三.1)** — seed42=0.738, seed123=0.735, seed789=0.742; vs D1 DPM++ 3-seed test 0.737, Δ=+0.001 (noise, 与 val Δ=−0.002 方向一致: v-pred 不优于 x0-pred)
- 与 Dataset 2 结论一致 (D2 Δ=−0.002 val / Δ≈−0.001 test), 方向支持 R3.2: v-prediction 在低维 + shifted schedule 下不优于 x0-prediction
- checkpoint 已同步至 ross (best_coco_bbox_mAP_epoch_{72,57,84}.pth, seed123/789 于 2026-08-03 从 workstation 同步)
...
- **v-prediction 3-seed 均值 mAP=0.857 ± 0.0015 (val)** (seed42=0.855 / seed123=0.858 / seed789=0.857, 均 val), vs +DPM-Solver++ baseline:
  ...
  -- **test: 0.858±0.003 (3-seed, 补跑 2026-08-03, 见 §十三.1 D2 3-seed test 表)** — seed42=0.855, seed123=0.860, seed789=0.858; vs D2 DPM++ test 0.859 (seed42), Δ≈−0.001 (noise)
- **方向性支持 R3.2 (3-seed 验证完成, val+test 双口径)**: v-prediction 3-seed 均值 (0.857 val / 0.858 test) 低于 x0-prediction 3-seed 均值 (0.859 val / 0.859 test seed42), Δ=−0.002 val / Δ≈−0.001 test 方向一致, 证实 v-prediction 在低维 (d=4) + shifted schedule (s=3.0) 下劣于 x0-prediction
...
- **✅ 双数据集验证完成 (2026-08-02 val + 2026-08-03 test)**: Dataset 1 3-seed 均值 0.745±0.004 val / 0.738±0.004 test, Δ=−0.002 val / +0.001 test; Dataset 2 3-seed 均值 0.857±0.0015 val / 0.858±0.003 test, Δ=−0.002 val / −0.001 test (均在 noise 范围, 方向一致)
...
> 数据源: [test_eval_per_size_20260730_204840.json](file:///home/linkst/workspace/projects/chromosome-kd/work_dirs/diagnosis/test_eval_per_size_20260730_204840.json) (12 模型, 含 per-class AP, 2026-07-30) + [test_eval_3seed_gap_20260803_022651.json](file:///home/linkst/workspace/projects/chromosome-kd/work_dirs/diagnosis/test_eval_3seed_gap_20260803_022651.json) (12 模型 3-seed 补跑, 2026-08-03); 详见 §十三.5 Dataset 1 SOTA val+test 对照表

##### 主路线模型 (标准增强, 3-seed test 补全)

| 模型 | Val mAP | Test mAP | Δ (test−val) | 备注 |
|------|:---------:|:--------:|:----------:|------|
...
| **KaryoFlow RF+Heun (3-seed mean)** | **0.746±0.001** | **0.737±0.002** | **−0.009** | — |
| KaryoFlow +DPM-Solver++ (seed42) | 0.746 | 0.739 | −0.007 | val best@ep49 |
| KaryoFlow +DPM-Solver++ (seed123) | 0.748 | 0.738 | −0.010 | val best@ep85; test 补跑 2026-08-03 |
| KaryoFlow +DPM-Solver++ (seed789) | 0.746 | 0.735 | −0.011 | val best@ep72; test 补跑 2026-08-03 |
| **KaryoFlow +DPM-Solver++ (3-seed mean)** | **0.747±0.001** | **0.737±0.002** | **−0.010** | DPM++=Heun on test (Δ=0.000), 与 val Δ=+0.001 一致 |
| KaryoFlow +Stoch. Coupling (seed42, reproduce_0751) | 0.753 | 0.740 | −0.013 | val best@ep59 (reproduce_0751_stochot_eps5_v2) |
| KaryoFlow +Stoch. Coupling (3-seed, multi_seed/stochot_eps5_old) | 0.747±0.002 | 0.738±0.004 | −0.009 | test 补跑 2026-08-03; seed42=0.736/123=0.735/789=0.742 |
| KaryoFlow v-prediction (3-seed) | 0.745±0.004 | 0.738±0.004 | −0.007 | test 补跑 2026-08-03; seed42=0.738/123=0.735/789=0.742 |
...
| **DiffusionDet DDPM (3-seed mean)** | **0.729±0.003** | **0.719±0.003** | **−0.010** | — |
...
##### OT Collapse 耦合消融 (标准增强, test 评估 2026-08-03)

> 数据源: [test_eval_3seed_gap_20260803_022651.json](file:///home/linkst/workspace/projects/chromosome-kd/work_dirs/diagnosis/test_eval_3seed_gap_20260803_022651.json) (StochOT 3-seed) + [hard_ot_aug_test_20260804_130041.json](file:///home/linkst/workspace/projects/chromosome-kd/work_dirs/diagnosis/hard_ot_aug_test_20260804_130041.json) (Hard OT 3-seed); test split 220 图

| 模型 | Val mAP | Test mAP | Δ (test−val) | seed42 / seed123 / seed789 (test) |
|------|:---------:|:--------:|:----------:|------|
| Hard OT | 0.748±0.001 (3-seed) | 0.737±0.002 (3-seed) | −0.011 | 0.738 / 0.735 / 0.739 |
| Random (AdaLN) | 0.746±0.001 (3-seed) | 0.737±0.002 (3-seed) | −0.009 | 0.739 / 0.735 / 0.737 |
| StochOT ε=5 | 0.747±0.002 (3-seed) | 0.738±0.004 (3-seed) | −0.009 | 0.736 / 0.735 / 0.742 |
| **Δ (Hard OT − Random)** | **+0.002 (ns)** | **+0.000 (ns)** | — | 标准增强下无 OT 坍缩 |
...
| **OT Collapse + Stoch. Coupling (§二)** | 低维 d=4 OT 坍缩形式化 + 标准增强补偿 + StochOT 训练稳定性 | 低维触发 / 高 K 加剧 / 小训练集理论风险 | 标准增强: D1 三耦合策略 Δ≤0.002 (ns, val+test 3-seed); D2 Hard OT 3-seed Δ=+0.001 (ns) + 4.6× 平滑; 理论上界 $\Delta H \ge 0.999\log K$ 仍成立 | ✅ 完成 |
...
| **v-prediction 对照 (§八)** | 验证低维 + shifted schedule 下 x0-prediction 优势 | 预防"为何不用 v-prediction"质疑 (RF 原文偏好) | D2: 3-seed 0.857±0.0015 val / 0.858±0.003 test, Δ=−0.002 val / −0.001 test; D1: 3-seed 0.745±0.004 val / 0.738±0.004 test, Δ=−0.002 val / +0.001 test (双数据集 val+test 方向一致, baseline=+DPM-Solver++ 3-seed mean 0.747/0.859 val) | ✅ 完成 (双数据集 3-seed val+test) |
...
| OT Collapse + Stoch. Coupling (§二) | ✓ Stoch vs Random +0.0001 (ns) + 4.6× 平滑 | ✓ 三耦合策略 Δ≤0.001 (ns, val+test 3-seed); 理论上界 $\Delta H \ge 0.999\log K$ | ✅ 完成 (标准增强统一) | — |
...
| Stoch vs Random (Dataset 1, 标准增强) | +0.001 | ns | 3-seed |
| Hard OT vs Random (Dataset 1, 标准增强) | +0.001 | ns | 2-3 seed |
