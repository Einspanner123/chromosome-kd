...
>
> 📌 **数据集说明**: 除特别标注 24obj 数据集的实验外, 多数结论基于旧数据集 Chromosome20240904 (chromo, mAP≈0.72-0.75), baseline = `rf_heun_adaln` = 0.746 ± 0.001 (3 seeds)。24obj 数据集 (mAP 量级 0.77-0.87) 实验单独标注, 不可与 chromo 数据集对比。
>
...
|--------|----------|-----|---------|------|
| chromo (Chromosome20240904) | rf_heun_adaln (3 seeds) | **0.746 ± 0.001** | ldmdet-ablation/rf_heun_adaln_seed* | 多数证伪实验对照 |
| 24obj (24_chromosomes_object) | A3 DPM-Solver++ (3 seeds) | **0.859 ± 0.003** | ldmdet-mainline-ablation-24obj | IO1-IO5 / N_cascade / Flow Matching 对照 |
...
E4.3 OLD (usnvd63f, 0.752) 早期声称 SCRF 有效, 但核查备份 head.py 发现 ScaleConditionedRF **未真正集成到 head.py**, 0.752 全部来自 OTFlowCoupling + 种子方差。
...
- E4.3 OLD (ScaleConditionedRF + OTFlowCoupling argmax eps=1.0, SCRF **未集成**)
...
- E4.3-tune eps=2.0 OLD (SCRF **未集成**, eps 调优)
...
- E4.3-tune eps=2.0 NEW (SCRF **真正集成**, TDD 红绿重构后) ⛔ 证伪
...
- 3-seed 复现 (E4.3 eps=2.0 配置, 验证可复现性, SCRF **未集成**)
...
|------|-----|----------|---|----|-----------|--------------|-------------|
| reproduce_0751_stochot_eps5_v2 | **0.753** | sinkhorn_stochastic | 5.0 | 2 | 无 | — | Stochastic OT ε=5 |
| scheme_C1_5_mixed_rel_l1_lam015 | **0.752** | sinkhorn_stochastic | 5.0 | 2 | 无 | — | + mixed_relative_l1 损失 |
| nonlinear_trajectory E4.3 (usnvd63f) | **0.752** | ot_flow (argmax) | 1.0 | 4 | 有 | ❌ **未集成** | ot_flow coupling |
...
4 个配置均未超越 baseline 0.746, 简单 frozen (E6.2) 优于复杂增强 (E6.3/E6.4)。

#### 实验证明目的

验证不同 FBM 架构 (frozen / enhanced / crossattn) 是否能超越 baseline 0.746。

- E6.2 frozen (FBM alpha 可学习 + UNet 全冻结)
...
- E6.3 enhanced (per-channel gate + GroupNorm + UNet 部分解冻)
...
- E6.3b frozen_enhanced (同 E6.3 但 UNet 全冻结, 隔离 FBM 架构效果)
...
- E6.4 crossattn (Cross-Attention FBM + UNet 部分解冻 + zero-init gamma)
...
1. **E6.4 gamma 零初始化致 attention 路径未激活**: zero-init gamma 使 cross-attention 输出在初始阶段被完全抑制, 梯度信号微弱, 路径无法被训练, 实际退化为 simple gate (与 E6.2 frozen 等价)。后续改进建议: 非零 gamma 初始化 (如 0.1) 或移除 gamma。
2. **E6.3 UNet 部分解冻破坏生成模型先验**: UNet 在检测损失下微调, 偏离了原始生成模型学到的形态先验, 注入的特征反而引入噪声。
3. **FBM 架构复杂度反比于性能**: E6.2 (最简单) > E6.4 > E6.3 > E6.3b, 表明 FBM 的价值不在于架构复杂度, 而在于生成模型特征本身与检测任务的兼容性不足。
...
验证 CFM 替代 RF 是否能超越 24obj baseline A3 (0.859)。

- Flow Matching Detection (CFM 训练)
  -- 数据集: 24obj
  -- SwanLab: ldmdet-frontier-directions
  -- mAP=0.823 (Δ=−0.033 vs A3 baseline 0.859, 24obj 数据集)
  -- 对应论文 Appendix B "flow-matching-det" 方向
...
## 四、N_cascade e2e (重训 cascade head 数量, 证伪)
...
- N_cascade e2e (减小 H, 端到端重训)
  -- 数据集: 24obj
  -- SwanLab: ldmdet-frontier-directions
  -- mAP=0.684 (Δ=−0.172 vs A3 baseline 0.859, 24obj 数据集, ⛔ 证伪, 性能坍塌)
  -- 对应论文 Appendix B "N_cascade e2e" 方向
...
在 A3 checkpoint (DPM-Solver++ 4 步 + Top-K 剪枝) 上不重训, 仅推理时优化以减少 NFE / 加速推理。5 个方向 (IO1, IO2, IO4, IO5; IO3 未实验) 均证伪。
...
所有实验均在 A3 checkpoint 上推理时评估, 无重训。

- IO1 adaptive step (根据 x0_pred 收敛提前终止)
...
- IO2 draft-verify (早期步骤 draft, 后期 verify)
...
- IO4 head early-exit (cascade head 提前退出)
...
- IO5 RoI feature cache (缓存 RoI 特征避免重复计算)
...
1. **推理时优化的根本限制**: 所有 IO 方向都依赖"早期步骤信息足以决策", 但 RF 在 t≈1 时模型输入近乎纯噪声, 任何基于 x0_pred 的决策都不可靠。
2. **与 R1 直线度指标的区分**: R1 (theory_analysis_RF_DPM.md §1.6) 仅观测 η_str 不做决策, 避开了 IO1 的失败模式; IO1 依赖 x0_pred 做决策, 触发失败。
3. **cascade head 不可精简**: S1 理论分析 (theory_analysis_RF_DPM.md §2.5) 预测 H 与 S 不可简单交换, IO4 实验证实每个 head 都必要。
4. **box renewal 与 cache 互斥**: box renewal 是 LDMDet 的核心机制 (低置信度 proposal 重置为噪声), 但它使 box 位置每步大幅变化, 与任何基于位置稳定性的缓存策略 (IO5) 互斥。
...
- Direction B (DecoupledHead 替代共享特征路径)
...
2. **小数据集下多任务正则化更重要**: chromo 数据集仅 1540 张训练图, 共享路径的多任务损失提供天然正则化, 解耦后模型过拟合风险增加。
3. **与 Cascade R-CNN 设计哲学冲突**: Cascade R-CNN 的 cascade head 共享特征路径正是利用多任务联合学习, Direction B 的解耦设计与该哲学冲突。

---

## 七、Direction C/F/A/E (未启动废弃)
...
验证 h_velocity_loss 变体是否能提升 24obj baseline。
...
FBM simple gate 源预训练, 期望在 24obj 源预训练阶段验证 simple gate FBM 架构的可行性, 为后续 few-shot 跨数据集微调提供基础。

### 失败状态

#### 实验证明目的

验证 FBM SimpleGate 在 24obj 源预训练阶段是否能达到与 LDMDet SOTA 相当的精度。
...
3. **与 E6.2 frozen 一致**: E6.2 frozen (FBM alpha 可学习 + UNet 全冻结) 在 chromo 数据集上 mAP=0.737 (Δ=−0.009), 同样是 simple gate 架构, 同样未超越 baseline, 证实 simple gate FBM 架构本身的有效性不足。
...
基于 SOTA 模型 (rf_heun_adaln + Sinkhorn Stochastic OT, 0.746) 的瓶颈分析, 针对分类损失、回归损失、采样策略等瓶颈点设计消融, 期望找到能超越 0.746 的配置。
...
- class_balanced_sampling (类别平衡采样) ⛔ FAILED
  ...
  -- 对应 Direction E (ClassBalanced) 的实验性验证, 证实该方向不可行
...
- proposals_100 (100 proposals 替代 500) ⛔ FAILED
  ...
  -- 状态: SIGKILL, 无有效 mAP
  -- 对应 Direction F (StructuredPrior + 100 proposals) 的实验性验证, 证实该方向不可行
...
4. **类别平衡采样内存溢出**: class_balanced_sampling 触发 SIGKILL, 可能是加权采样器在小批量 (bs=2) 下内存占用过高。Direction E 的设想虽合理, 但实现层面不可行。
5. **100 proposals 容量不足**: proposals_100 触发 SIGKILL, 可能是 proposal 数量减少后某些维度不匹配。即使能训练, 24obj 数据集每张图约 46 个目标, 100 proposals 覆盖 46 + 重叠冗余时容量紧张 (与 Top-K K=100 掉点 −0.010 同源)。
6. **架构天花板约 0.75**: 7 个消融均未超越 0.746, 证实 chromo 数据集上架构天花板约 0.75, 需换数据集 (24obj) 或换范式 (RF + DPM-Solver++) 才能突破。
...
- cspnext_l_rf_heun_adaln_stochot (CSPNeXt-L 主干)
  ...
  -- mAP=0.730 (Δ=−0.016)
  -- 失败原因: CSPNeXt-L 主干在 chromo 数据集上表现不如 ResNet-50, 可能是预训练权重与染色体数据不兼容
...
4. **CSPNeXt-L 不适配小数据**: cspnext_l (0.730) 低于 baseline, CSPNeXt-L 主干参数量更大, 在 chromo 小数据集 (1540 张) 上过拟合。
...
|------|-----|---------------|--------|------------------|------|
| ScaleConditionedRF (E4.3 eps2 NEW) | 0.741 | −0.005 | chromo | ldmdet-ablation | ⛔ 证伪 |
| FBM E6.2 frozen | 0.737 | −0.009 | chromo | ldmdet-ablation | ⛔ 证伪 |
| FBM E6.3 enhanced | 0.703 | −0.043 | chromo | ldmdet-ablation | ⛔ 证伪 |
| FBM E6.3b frozen_enhanced | 0.696 | −0.050 | chromo | ldmdet-ablation | ⛔ 证伪 |
| FBM E6.4 crossattn | 0.733 | −0.013 | chromo | ldmdet-ablation | ⛔ 证伪 |
| Flow Matching Detection | 0.823 | −0.033 | 24obj | ldmdet-frontier-directions | ⛔ 证伪 |
| N_cascade e2e | 0.684 | −0.172 | 24obj | ldmdet-frontier-directions | ⛔ 证伪 |
| IO1 adaptive step | — | — | 24obj | — | ⛔ 证伪 |
...
| scale_aware_loss | 0.742 | −0.004 | chromo | ldmdet-ablation | ⛔ 证伪 |
| relative_l1_loss | 0.740 | −0.006 | chromo | ldmdet-ablation | ⛔ 证伪 |
| high_cls_weight | 0.739 | −0.007 | chromo | ldmdet-ablation | ⛔ 证伪 |
| high_giou_weight | 0.737 | −0.009 | chromo | ldmdet-ablation | ⛔ 证伪 |
| no_box_renewal | 0.730 | −0.016 | chromo | ldmdet-ablation | ⛔ 消融证伪 |
| class_balanced_sampling | — | — | chromo | ldmdet-ablation | ⛔ SIGKILL |
| proposals_100 | — | — | chromo | ldmdet-ablation | ⛔ SIGKILL |
| multi_seed/rf_heun_adaln (无 aug) | 0.712 | −0.034 | chromo | chromosome-kd | ⛔ 简化非标准 |
| multi_seed/rf_heun_adaln_bs4 | 0.702 | −0.044 | chromo | — | ⛔ 简化非标准 |
| multi_seed/hard_ot (无 aug) | 0.705 | −0.041 | chromo | chromosome-kd | ⛔ 简化非标准 |
| debug_train_final | 0.525 | −0.221 | chromo | — | ⛔ 调试 |
| stability/warm_restart_* | 0.714-0.728 | −0.018~-0.032 | chromo | chromosome-kd | ⛔ 证伪 |
| scheme_a_dinov2_s | 0.502-0.676 | −0.070~-0.244 | chromo | chromosome-kd | ⛔ 全失败 |
| scheme_E_bifpn | 0.744 | −0.002 | chromo | chromosome-kd | 🟠 持平, 未采用 |
| cspnext_l_rf_heun_adaln_stochot | 0.730 | −0.016 | chromo | chromosome-kd | ⛔ 证伪 |
