...
- 从 $t=0$（数据 $x_0 \\sim p\_{\\text{data}}$）到 $t=T$（近似纯噪声 $x_T \\sim \\mathcal{N}(0, I)$）
- 这是一个**随机扩散过程**，转移核为高斯：
...
用神经网络 $s\_\\theta(x_t, t)$ 来近似分数函数：

$$\\mathcal{L}*{\\text{Score}} = \\mathbb{E}*{t, x_0, \\epsilon}\\left\[\\left| s\_\\theta(x_t, t) - \\nabla\_{x_t} \\log q(x_t|x_0) \\right|^2\\right\]$$

由于 $q(x_t|x_0) = \\mathcal{N}(\\alpha_t x_0, \\sigma_t^2 I)$，等价于**预测噪声**：

$$\\boxed{\\mathcal{L}*{\\text{DDPM}} = \\mathbb{E}*{t, x_0, \\epsilon}\\left\[\\left| \\epsilon\_\\theta(x_t, t) - \\epsilon \\right|^2\\right\]}$$
...
其中 $x_0 \\sim \\mathcal{N}(0, I)$（噪声），$x_1 \\sim p\_{\\text{data}}$。
...
$$\\boxed{\\mathcal{L}*{\\text{FM}} = \\mathbb{E}*{t \\sim U\[0,1\],, x_0 \\sim \\mathcal{N}(0,I),, x_1 \\sim p\_{\\text{data}}}\\left\[\\left| v\_\\theta(x_t, t) - (x_1 - x_0) \\right|^2\\right\]}$$
...
$$x\_{t+\\Delta t} = x_t + v\_\\theta(x_t, t),\\Delta t$$
