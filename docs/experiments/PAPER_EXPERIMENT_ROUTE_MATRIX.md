...
- YAML SHA-256：`d14afe4b434a7e5904c26aa3df8c1aa07089fb632de5f8bea57552997cf2705b`
- 数据库 artifact：`paper-route-matrix-v2-d14afe4b434a`
...
- 实验组：53；展开运行：287。
- 状态分布：ARCHIVED_NONCOMPARABLE=1, BLOCKED_CHECKPOINTS=1, BLOCKED_PARENT=3, BLOCKED_PREDICTIONS=5, COMPLETED_CENTRAL_TEST_VERIFIED=2, COMPLETED_DESCRIPTIVE=1, COMPLETED_DIAGNOSTIC=2, COMPLETED_EVIDENCE_ONLY=1, COMPLETED_FIXED_CHECKPOINT=2, COMPLETED_PARTIAL=1, COMPLETED_POINT_ESTIMATE=5, COMPLETED_TEST3_VERIFIED=4, COMPLETED_VERIFIED=2, DEFERRED_UNTIL_D1_COMPLETE=12, PARTIAL_LEGACY=1, PLANNED=6, QUEUED_SERIAL_ROSS_0_OF_30_COMPLETE=1, QUEUED_SERIAL_ROSS_32_OF_36_COMPLETE=1, REVIEW_EXISTING_VALIDATION_SELECTION=1, TRAIN3_COMPLETE_TEST_QUEUED_2_OF_3_COMPLETE=1。
...
- `D1I.INF.solver_steps`：科学因素=3 parents x {Euler,Heun,DPM++} x {1,2,3,4}; report steps and NFE；验收=same checkpoint per parent; common test; no cross-checkpoint causal claim；论文用途=Solver/step accuracy-efficiency ablation；备注=32/36 fixed-parent solver/step held-out test evaluations passed the evidence/database audit; remaining tasks are serialized on Ross A6000.
