...
- YAML SHA-256：`d6b27286359e69e83d84a2ad1d49d985aeafbd8ea7821f859ac8c43e9d4a8f21`
- 数据库 artifact：`paper-route-matrix-v2-d6b27286359e`
- 实验组：46；展开运行：261。
...
| `D2.SOTA.diffusiondet.missing_train2` | diffusiondet | full_train / 2 | train=123,789; infer=42 | **PLANNED** | READY | `experiments/configs/methods/diffusiondet_ddpm.py` | 123=workstation:A5000:0;789=ross:A6000:0 (A4000 for lighter model if exact config fits) | `work_dirs/v2/d2_sota_completion/diffusiondet_ddpm_r50/trainseed_{training_seed}` | `route:D2.SOTA.diffusiondet.missing_train2; sota_d2_train3_completion` |
...
| `D2.SOTA.dino_r50.missing_train2` | dino_r50 | full_train / 2 | train=123,789; infer=42 | **PLANNED** | READY | `experiments/configs/methods/dino_r50.py` | 123=workstation:A5000:0;789=ross:A6000:0 (A4000 for lighter model if exact config fits) | `work_dirs/v2/d2_sota_completion/dino_r50/trainseed_{training_seed}` | `route:D2.SOTA.dino_r50.missing_train2; sota_d2_train3_completion` |
...
| `D2.SOTA.rtmdet_l.missing_train2` | rtmdet_l | full_train / 2 | train=123,789; infer=42 | **PLANNED** | READY | `experiments/configs/methods/rtmdet_l.py` | 123=workstation:A5000:0;789=ross:A6000:0 (A4000 for lighter model if exact config fits) | `work_dirs/v2/d2_sota_completion/rtmdet_l/trainseed_{training_seed}` | `route:D2.SOTA.rtmdet_l.missing_train2; sota_d2_train3_completion` |
...
| `D2.SOTA.cascade_rcnn_r50.missing_train2` | cascade_rcnn_r50 | full_train / 2 | train=123,789; infer=42 | **PLANNED** | READY | `experiments/configs/methods/cascade_rcnn_r50.py` | 123=workstation:A5000:0;789=ross:A6000:0 (A4000 for lighter model if exact config fits) | `work_dirs/v2/d2_sota_completion/cascade_rcnn_r50/trainseed_{training_seed}` | `route:D2.SOTA.cascade_rcnn_r50.missing_train2; sota_d2_train3_completion` |
...
| `D2.SOTA.yolox_s.missing_train2` | yolox_s | full_train / 2 | train=123,789; infer=42 | **PLANNED** | READY | `experiments/configs/methods/yolox_s.py` | 123=workstation:A5000:0;789=ross:A6000:0 (A4000 for lighter model if exact config fits) | `work_dirs/v2/d2_sota_completion/yolox_s/trainseed_{training_seed}` | `route:D2.SOTA.yolox_s.missing_train2; sota_d2_train3_completion` |

- `D2.SOTA.karyoflow_train3`：科学因素=RF; shifted t; AdaLN-Zero; DPM++4; K=500; 6 heads；验收=already verified: 3 checkpoint SHA; 1000-image test；论文用途=Main D2 KaryoFlow mean；备注=
- `D2.SOTA.diffusiondet.existing`：科学因素=published table checkpoint; six test metrics；验收=must not be labeled three-training-seed；论文用途=D2 point estimate only；备注=
- `D2.SOTA.diffusiondet.missing_train2`：科学因素=same configuration and selection rule as existing checkpoint；解析配置=batch 2, 150 epochs, AdamW, lr=5e-05, config_id=v2.d2_sota_completion.diffusiondet_ddpm_r50.train；验收=actual framework seed recorded; distinct checkpoint SHA; 1000-image test；论文用途=Upgrade D2 baseline to three independent trainings；备注=
- `D2.SOTA.dino_r50.existing`：科学因素=published table checkpoint; six test metrics；验收=must not be labeled three-training-seed；论文用途=D2 point estimate only；备注=
- `D2.SOTA.dino_r50.missing_train2`：科学因素=same configuration and selection rule as existing checkpoint；解析配置=batch 2, 150 epochs, AdamW, lr=2.5e-05, config_id=v2.d2_sota_completion.dino_r50.train；验收=actual framework seed recorded; distinct checkpoint SHA; 1000-image test；论文用途=Upgrade D2 baseline to three independent trainings；备注=
- `D2.SOTA.rtmdet_l.existing`：科学因素=published table checkpoint; six test metrics；验收=must not be labeled three-training-seed；论文用途=D2 point estimate only；备注=
- `D2.SOTA.rtmdet_l.missing_train2`：科学因素=same configuration and selection rule as existing checkpoint；解析配置=batch 2, 150 epochs, AdamW, lr=0.0001, config_id=v2.d2_sota_completion.rtmdet_l.train；验收=actual framework seed recorded; distinct checkpoint SHA; 1000-image test；论文用途=Upgrade D2 baseline to three independent trainings；备注=
- `D2.SOTA.cascade_rcnn_r50.existing`：科学因素=published table checkpoint; six test metrics；验收=must not be labeled three-training-seed；论文用途=D2 point estimate only；备注=
- `D2.SOTA.cascade_rcnn_r50.missing_train2`：科学因素=same configuration and selection rule as existing checkpoint；解析配置=batch 2, 150 epochs, AdamW, lr=0.0001, config_id=v2.d2_sota_completion.cascade_rcnn_r50.train；验收=actual framework seed recorded; distinct checkpoint SHA; 1000-image test；论文用途=Upgrade D2 baseline to three independent trainings；备注=
- `D2.SOTA.yolox_s.existing`：科学因素=published table checkpoint; six test metrics；验收=must not be labeled three-training-seed；论文用途=D2 point estimate only；备注=
- `D2.SOTA.yolox_s.missing_train2`：科学因素=same configuration and selection rule as existing checkpoint；解析配置=batch 2, 200 epochs, AdamW, lr=0.0001, config_id=v2.d2_sota_completion.yolox_s.train；验收=actual framework seed recorded; distinct checkpoint SHA; 1000-image test；论文用途=Upgrade D2 baseline to three independent trainings；备注=
#### Generation
...
| `D2.ABL.strict.G3` | RF_shifted_AdaLNZero | full_train / 3 | train=42,123,789; infer=42 | **PLANNED** | READY | `experiments/configs/methods/karyoflow.py` | historical ross/workstation | `work_dirs/v2/d2_generation/karyoflow_r50/trainseed_{training_seed}` | `route:D2.ABL.strict.G3; strict_generation_d2_test` |
...
- `D2.ABL.strict.G3`：科学因素=RF; shifted t; AdaLN-Zero; DPM++4；解析配置=batch 2, 150 epochs, AdamW, lr=5e-05, config_id=v2.d2_generation.karyoflow_r50.train；验收=confirm optimizer/batch/selection compatibility with new G0-G2；论文用途=Strict chain endpoint; reuse only after config compatibility audit；备注=
...
- `D2.INF.solver_steps.train3`：科学因素=3 independent parents x 3 solvers x 4 steps；验收=aggregate one value per training parent；论文用途=Training-robust solver ablation；备注=
...
- `D2.INF.topk_renewal.train3`：科学因素=3 parents x 5 K x renewal off/on；验收=aggregate by training parent；论文用途=Training-robust identity analysis；备注=
#### Decision
...
- `D2.SOTA.karyoflow_lqcr_train3`：科学因素=parent-matched final-only quality head; beta=2；验收=already verified: shared detector weights unchanged；论文用途=Main D2 LQCR paired effect；备注=
...
- `D2.DEP.distill_h3.train3`：科学因素=one H3 student per KaryoFlow parent; identical mapping/loss；解析配置=batch 2, 50 epochs, AdamW, lr=1e-05, config_id=v2.d2_distill.karyoflow_ot_h3_distill_r50.train；验收=three parent-matched students; A6000 latency only；论文用途=Training-robust deployment evidence；备注=
...
- `D2.DEP.speed`：科学因素=A6000: RF-Heun; DPM++ K=100/200/300/500; LQCR; H3; repeat after final checkpoints；验收=repeat final selected points with exclusive GPU and frozen timing boundary；论文用途=Speed-accuracy figure/table；备注=Historical latency is valid under its recorded protocol but PARTIAL against the strict rerun protocol.
