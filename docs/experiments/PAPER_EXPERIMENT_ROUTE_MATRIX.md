...
- YAML SHA-256：`22667f71a0b3eb24e2e6d0529b8d60bc150e2d96652db233b44bf823096e4b69`
- 数据库 artifact：`paper-route-matrix-v2-22667f71a0b3`
...
- 实验组：53；展开运行：287。
- 状态分布：ARCHIVED_NONCOMPARABLE=1, BLOCKED_CHECKPOINTS=1, BLOCKED_PARENT=3, BLOCKED_PREDICTIONS=5, COMPLETED_CENTRAL_TEST_VERIFIED=2, COMPLETED_DESCRIPTIVE=1, COMPLETED_DIAGNOSTIC=2, COMPLETED_EVIDENCE_ONLY=1, COMPLETED_FIXED_CHECKPOINT=2, COMPLETED_PARTIAL=1, COMPLETED_POINT_ESTIMATE=5, COMPLETED_TEST3_VERIFIED=4, COMPLETED_TEST_VERIFIED=1, COMPLETED_VERIFIED=2, DEFERRED_UNTIL_D1_COMPLETE=7, PARTIAL_LEGACY=1, PLANNED=6, QUEUED_SERIAL_ROSS_23_OF_30_COMPLETE=1, QUEUED_WORKSTATION_0_OF_2_COMPLETE=3, REVIEW_EXISTING_VALIDATION_SELECTION=1, TRAIN3_COMPLETE_TEST_QUEUED_2_OF_3_COMPLETE=1, TRAINING_WORKSTATION_0_OF_2_COMPLETE=2。
...
- `D1I.SOTA.diffusiondet`：科学因素=DDPM; DDIM 1-step; linear t; scale-shift; K=500; 6 heads；解析配置=batch 2, 150 epochs, AdamW, lr=5e-05, config_id=v2.d1.diffusiondet_ddpm_r50.train；验收=3 distinct checkpoint SHA; same manifest/config/protocol; six test metrics；论文用途=Main two-cohort detector table；备注=All three independent G0 training runs completed. Held-out test evidence: 3/3 passed evaluation, immutable evidence validation, database import, and global audit; remaining evaluations run serially on Ross A6000.
...
- `D1I.ABL.G1`：科学因素=RF; Euler/locked validation protocol; linear t; scale-shift；解析配置=batch 2, 150 epochs, AdamW, lr=5e-05, config_id=v2.d1_generation.strict_g1_rf_linear_scaleshift_r50.train；验收=only named factor changes; batch/optimizer/augmentation/selection fixed；论文用途=Strict incremental generation ablation；备注=All three independent G1 training runs completed. Held-out test evidence: 3/3 passed evaluation, immutable evidence validation, database import, and global audit; remaining evaluations run serially on Ross A6000.
- `D1I.ABL.G2`：科学因素=RF; shifted t (shift=3); scale-shift；解析配置=batch 2, 150 epochs, AdamW, lr=5e-05, config_id=v2.d1_generation.strict_g2_rf_shifted_scaleshift_r50.train；验收=only named factor changes; batch/optimizer/augmentation/selection fixed；论文用途=Strict incremental generation ablation；备注=All three independent G2 training runs completed. Held-out test evidence: 3/3 passed evaluation, immutable evidence validation, database import, and global audit; remaining evaluations run serially on Ross A6000.
- `D1I.ABL.G3`：科学因素=RF; shifted t; AdaLN-Zero; Euler 1-step validation；解析配置=batch 2, 150 epochs, AdamW, lr=5e-05, config_id=v2.d1_generation.strict_g3_rf_shifted_adaln_zero_r50.train；验收=only time conditioning changes from G2; common validation protocol；论文用途=Strict one-factor generation ablation；备注=All three independent G3 training runs completed. Held-out test evidence: 2/3 passed evaluation, immutable evidence validation, database import, and global audit; remaining evaluations run serially on Ross A6000.
- `D1I.INF.solver_steps`：科学因素=3 parents x {Euler,Heun,DPM++} x {1,2,3,4}; report steps and NFE；验收=same checkpoint per parent; common test; no cross-checkpoint causal claim；论文用途=Solver/step accuracy-efficiency ablation；备注=36/36 fixed-parent solver/step held-out test evaluations passed the evidence/database audit; remaining tasks are serialized on Ross A6000.
- `D1I.INF.topk_renewal`：科学因素=3 parents x K={100,150,200,300,500} x renewal={off,on}；验收=same checkpoint/seed; renewal-off candidate identity audit；论文用途=Identity/renewal and candidate-budget analysis；备注=23/30 fixed-parent Top-K/renewal held-out test evaluations passed the evidence/database audit; remaining tasks are serialized on Ross A6000.
#### Decision
...
| `D2.SOTA.diffusiondet.train3_supplement` | diffusiondet | full_train / 3 | train=42,123,789; infer=42 | **QUEUED_WORKSTATION_0_OF_2_COMPLETE** | READY | `experiments/configs/methods/diffusiondet_ddpm.py` | seeds 123/789 on workstation; heavy models on A5000 and lighter models on A4000; held-out tests and evidence import centralized on Ross | `work_dirs/v2/d2_sota_completion/diffusiondet_ddpm_r50/trainseed_{training_seed}` | `route:D2.SOTA.diffusiondet.train3_supplement; sota_d2_train3_completion` |
...
| `D2.SOTA.rtmdet_l.train3_supplement` | rtmdet_l | full_train / 3 | train=42,123,789; infer=42 | **QUEUED_WORKSTATION_0_OF_2_COMPLETE** | READY | `experiments/configs/methods/rtmdet_l.py` | seeds 123/789 on workstation; heavy models on A5000 and lighter models on A4000; held-out tests and evidence import centralized on Ross | `work_dirs/v2/d2_sota_completion/rtmdet_l/trainseed_{training_seed}` | `route:D2.SOTA.rtmdet_l.train3_supplement; sota_d2_train3_completion` |
...
| `D2.SOTA.cascade_rcnn_r50.train3_supplement` | cascade_rcnn_r50 | full_train / 3 | train=42,123,789; infer=42 | **QUEUED_WORKSTATION_0_OF_2_COMPLETE** | READY | `experiments/configs/methods/cascade_rcnn_r50.py` | seeds 123/789 on workstation; heavy models on A5000 and lighter models on A4000; held-out tests and evidence import centralized on Ross | `work_dirs/v2/d2_sota_completion/cascade_rcnn_r50/trainseed_{training_seed}` | `route:D2.SOTA.cascade_rcnn_r50.train3_supplement; sota_d2_train3_completion` |
...
| `D2.SOTA.yolox_s.train3_supplement` | yolox_s | full_train / 3 | train=42,123,789; infer=42 | **TRAINING_WORKSTATION_0_OF_2_COMPLETE** | READY | `experiments/configs/methods/yolox_s.py` | seeds 123/789 on workstation; heavy models on A5000 and lighter models on A4000; held-out tests and evidence import centralized on Ross | `work_dirs/v2/d2_sota_completion/yolox_s/trainseed_{training_seed}` | `route:D2.SOTA.yolox_s.train3_supplement; sota_d2_train3_completion` |
...
- `D2.SOTA.diffusiondet.train3_supplement`：科学因素=same configuration and selection rule as existing checkpoint；解析配置=batch 2, 150 epochs, AdamW, lr=5e-05, config_id=v2.d2_sota_completion.diffusiondet_ddpm_r50.train；验收=three independently trained models; six held-out-test metrics；论文用途=D2 three-training-run robustness supplement；备注=The historical seed-42 point and two newly scheduled independent runs form train3. New-run completion: 0/2; running=0, queued=2.
...
- `D2.SOTA.dino_r50.train3_supplement`：科学因素=same configuration and selection rule as existing checkpoint；解析配置=batch 2, 150 epochs, AdamW, lr=2.5e-05, config_id=v2.d2_sota_completion.dino_r50.train；验收=three independently trained models; six held-out-test metrics；论文用途=D2 three-training-run robustness supplement；备注=The historical seed-42 point and two newly scheduled independent runs form train3. New-run completion: 0/2; running=1, queued=1.
...
- `D2.SOTA.rtmdet_l.train3_supplement`：科学因素=same configuration and selection rule as existing checkpoint；解析配置=batch 2, 150 epochs, AdamW, lr=0.0001, config_id=v2.d2_sota_completion.rtmdet_l.train；验收=three independently trained models; six held-out-test metrics；论文用途=D2 three-training-run robustness supplement；备注=The historical seed-42 point and two newly scheduled independent runs form train3. New-run completion: 0/2; running=0, queued=2.
...
- `D2.SOTA.cascade_rcnn_r50.train3_supplement`：科学因素=same configuration and selection rule as existing checkpoint；解析配置=batch 2, 150 epochs, AdamW, lr=0.0001, config_id=v2.d2_sota_completion.cascade_rcnn_r50.train；验收=three independently trained models; six held-out-test metrics；论文用途=D2 three-training-run robustness supplement；备注=The historical seed-42 point and two newly scheduled independent runs form train3. New-run completion: 0/2; running=0, queued=2.
...
- `D2.SOTA.yolox_s.train3_supplement`：科学因素=same configuration and selection rule as existing checkpoint；解析配置=batch 2, 200 epochs, AdamW, lr=0.0001, config_id=v2.d2_sota_completion.yolox_s.train；验收=three independently trained models; six held-out-test metrics；论文用途=D2 three-training-run robustness supplement；备注=The historical seed-42 point and two newly scheduled independent runs form train3. New-run completion: 0/2; running=1, queued=1.
#### Generation
