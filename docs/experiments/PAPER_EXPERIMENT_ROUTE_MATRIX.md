...
- YAML SHA-256：`5e89d94fb559a99bb1bf839e16d612c40ab3784f1e623b4a77ee1af5adfb4c85`
- 数据库 artifact：`paper-route-matrix-v2-5e89d94fb559`
...
| `D2.SOTA.karyoflow_canonical_train3` | karyoflow | full_train / 3 | train=42,123,789; infer=42 | **PLANNED** | READY | `experiments/configs/methods/karyoflow.py` | 42=ross:A6000:0;123=workstation:A5000:0;789=workstation:A4000:1 | `work_dirs/v2/d2/karyoflow_r50/trainseed_{training_seed}` | `route:D2.SOTA.karyoflow_canonical_train3; sota_d2_canonical_test` |
...
- `D2.SOTA.karyoflow_canonical_train3`：科学因素=RF; shifted t; AdaLN-Zero; DPM++ 4-step; K=500; 6 heads; random coupling；解析配置=batch 2, 150 epochs, AdamW, lr=5e-05, config_id=v2.d2.karyoflow_r50.train；验收=three canonical random-coupling checkpoints; six held-out-test metrics；论文用途=Canonical D2 main detector table；备注=Deferred at completed epoch 3 after confirming that all paper D2 evidence follows the publisher-provided original split; retain only as an optional split-sensitivity experiment.
#### Generation
...
- `D2.ABL.strict.G3`：科学因素=RF; shifted t; AdaLN-Zero; Euler 1-step validation；解析配置=batch 2, 150 epochs, AdamW, lr=5e-05, config_id=v2.d2_generation.strict_g3_rf_shifted_adaln_zero_r50.train；验收=only time conditioning changes from G2; common validation protocol；论文用途=Cross-cohort strict one-factor generation ablation；备注=Strict one-factor AdaLN-Zero stage; DPM++ is evaluated separately on a fixed checkpoint.
...
| `D2.DEP.speed.canonical` | all_operating_points | benchmark / 1 | train=42,123,789; infer=none | **BLOCKED_CHECKPOINTS** | PROTOCOL_READY_PARENT_PENDING | `experiments/configs/methods/karyoflow.py` | 42=ross:A6000:0;123=workstation:A5000:0;789=workstation:A4000:1 | `results/v2/d2/benchmark/a6000/{variant}` | `route:D2.DEP.speed.canonical; speed_accuracy_d2_canonical` |
...
- `D2.DEP.speed.canonical`：科学因素=A6000: RF-Heun; DPM++ K=100/200/300/500; LQCR; H3; repeat after final checkpoints；验收=three A6000 repeats per final checkpoint; matching test accuracy; params and peak memory；论文用途=Final canonical speed-accuracy figure/table；备注=Canonical v2 seed 42 is active; seeds 123 and 789 remain planned.
