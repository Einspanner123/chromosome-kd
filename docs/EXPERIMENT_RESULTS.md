...
### 0.2b FPS / 延迟 Benchmark（RTX A6000, 2026-07-14）

> 脚本: `ldmdet/tools/benchmark_fps.py` | 输入: 512×512, batch=1 | warmup=10, iters=100
> 原始数据: `results/benchmark_fps_20260714_231841.{md,json}` (基础 6 模型) + `results/benchmark_fps_20260714_234842.{md,json}` (A4+IO3 组合)

| 模型 | 采样器 | 步数 | 延迟 (ms) | FPS | Backbone+Neck (ms) | Head (ms) | mAP |
|------|--------|:---:|----------:|----:|-------------------:|----------:|---:|
| A1 (RF+Heun) | Heun | 4 | 124.38 ± 3.38 | 8.0 | 5.80 | 118.58 | 0.856 |
| A3 (SOTA, Heun) | Heun | 4 | 128.35 ± 1.95 | 7.8 | 5.75 | 122.60 | 0.858 |
| **A4 (DPM-Solver++)** | **DPM++** | **4** | **75.03 ± 0.96** | **13.3** | **5.80** | **69.23** | **0.863** |
| IO3 K=300 (A3+) | Heun | 4 | 117.66 ± 2.65 | 8.5 | 5.71 | 111.95 | 0.857 |
| IO3 K=200 (A3+) | Heun | 4 | 114.37 ± 2.08 | 8.7 | 5.68 | 108.69 | 0.856 |
...
| RTMDet-L | — | — | 33.06 ± 0.80 | 30.3 | 12.70 | 20.36 | 0.869 |

**核心发现**：
1. **A4+IO3 K=300 是最快的 LDMDet 变体**: 71.27 ms / 14.0 FPS / mAP 0.861
   - vs A3 (Heun): **1.80x 加速** (128.35→71.27 ms), 精度 +0.003 (0.858→0.861)
   - vs A4 (DPM++): 1.05x 加速 (75.03→71.27 ms), 精度 -0.002 (0.863→0.861)
2. **A4 (DPM-Solver++) 精度速度双收益**: 75.03 ms / 13.3 FPS / mAP 0.863
   - vs A3 (Heun): **1.71x 加速** (128→75 ms) **且精度提升 +0.005** (0.858→0.863)
   - 原因: DPM-Solver++ 每步仅需 1 次模型调用, Heun 需 2 次 (4步×1=4 calls vs 4步×2=8 calls)
3. **A4+IO3 组合加速有限** (K=300: 1.05x, K=200: 1.06x, K=100: 1.08x vs A4):
   - DPM-Solver++ 每步仅 1 次调用, 剪枝只对第 2-4 步 (3/4 calls) 生效
   - K=300 最优性价比: 精度 -0.002, 速度 +0.7 FPS
   - K=100 精度下降明显 (-0.013), 不推荐
4. **IO3 在 Heun 上效果更显著** (A3+K300: 1.09x, A3+K200: 1.12x vs A3):
   - Heun 每步 2 次调用, 剪枝对第 2-4 步 (6/8 calls) 生效, 受益比例更高
5. **RTMDet-L 速度优势明显** (33 ms / 30.3 FPS), 但仅比 A4+K300 高 +0.008 mAP
6. **Backbone+Neck 占比小**: LDMDet ~5.8 ms (4.5-8%), Head 占 90%+ 延迟
