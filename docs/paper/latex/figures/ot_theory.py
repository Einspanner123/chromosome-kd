...
def panel_voronoi(ax: plt.Axes) -> None:
    ...
    ax.scatter(GT_POINTS[:, 0], GT_POINTS[:, 1], s=70, color=C_GT,
               marker="s", edgecolor="k", lw=0.7, zorder=5, label="GT boxes ($b_k$)")
    ax.scatter(noise[:, 0], noise[:, 1], s=40, color=C_NOISE,
    ...
    handles = [
        plt.Line2D([0], [0], color=C_OT, lw=1.4, label="OT assignment"),
        plt.Line2D([0], [0], color=C_RAND, lw=1.4, ls="--", label="Random assignment"),
        plt.Line2D([0], [0], color=C_VOR, lw=1.0, label="Voronoi cells"),
    ]
...
def panel_dh(ax: plt.Axes) -> None:
    ...
    x = np.arange(2)
    bars = ax.bar(x, values, width=0.5, color=colors, edgecolor="black", lw=0.7)

    for bar, v in zip(bars, values):
        ax.text(bar.get_x() + bar.get_width() / 2, v / 2,
                f"{v:.4f}", ha="center", va="center", fontsize=9,
                color="white", fontweight="bold")

    ax.set_xticks(x)
    ax.set_xticklabels(bar_labels, fontsize=9)
    ax.set_ylabel(r"Conditional entropy reduction $\Delta H$", fontsize=10)
    ax.set_ylim(0.0, 4.7)
    ...
    y_top = max(theory, empirical) + 0.3
    ax.annotate(
        "",
        xy=(0, theory + 0.1), xytext=(1, empirical + 0.1),
