...
At similar NFE, DPM-Solver++ 4-step (4 NFE, $0.863$) ≈ Heun 2-step (3 NFE, $0.863$) — equal accuracy, so DPM-Solver++ buys 43% fewer NFE at no cost. Crucially, at matched *step count* (4 vs 4), DPM-Solver++ is *more* accurate than Heun ($+0.006$ per-image mAP, Wilcoxon $p<10^{-6}$; Table 8), so its advantage is both computational and a small precision gain — not "purely computational" as one might infer from the matched-NFE comparison alone.
