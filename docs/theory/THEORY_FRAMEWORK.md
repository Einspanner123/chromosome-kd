...
> ⚠️ **暂时废弃（实验数值部分）**：本文档的理论推导（命题、证明、机制分析）仍然有效；但所有嵌入的实验验证数值（如 mAP 0.725-0.755、hard OT 0.735 < random 0.751 等）均基于旧数据集 Chromosome20240904（mAP≈0.72-0.75），24obj 数据集上的结论已更新（mAP 量级 0.77-0.87，A0-A4 主路线消融见 §7 标注后补充）。请勿将本文 chromo 数值用于论文写作。
...
> **代码审计 / 重跑标注（2026-05-15）**：该结论依赖 DDPM baseline。当前 `ldmdet/core/head.py::_ddim_step` 在 `t_next < 0` 时仍先访问 `self.alphas_cumprod[t_next]`，会触发 Python 负索引，影响 `ldmdet_baseline` 与 `ldmdet_baseline_step4` 的公平性。应先修复 `_ddim_step` 的终止步逻辑，再重跑 `ldmdet_baseline`、`ldmdet_baseline_step4` 以及 RF-vs-DDPM 公平对比。RF 本身的直线路径命题不受影响，但 `+0.8 mAP` 数值需要重验。
...
> ⚠️ **暂时废弃**：以下实验验证状态表基于旧数据集 Chromosome20240904（mAP≈0.72-0.75），24obj 数据集上的结论已更新。下表的实验数值（如 +0.8%、+1.4%、hard OT 0.735 < random 0.751 等）不可直接引用。
>
> **24obj 数据集 A0-A4 主路线消融（新）**：
>
> | 实验 | mAP | 说明 |
> |------|-----|------|
> | A0 baseline (Euler 1步, 无RF) | 0.774 | 基线 |
> | A1 +RF+Heun | 0.856 | +0.082 |
> | A2 +AdaLN-Zero | 0.856 | +0.000 |
> | A3 +StochOT eps5 | 0.858 | +0.002 |
> | A4 DPM-Solver++替换Heun | 0.863 | +0.005 |
>
> 24obj 耦合策略消融（3 seeds）：Random 0.860±0.001、GHSS 0.858±0.001、Sinkhorn Stochastic 0.856(1 seed)。
...
> ⚠️ **暂时废弃**：以下 KCEC 审计结论基于旧数据集 Chromosome20240904（mAP≈0.72-0.75，SOTA 0.753、KCEC V2 0.732 等），24obj 数据集上的结论已更新。KCEC 的结构性失败分析（耦合层与表征层信息截断）作为机制假说仍可参考，但具体 mAP 数值不可引用。
...
**诊断脚本**：`projects/LDMDet/tools/diagnose_kcec.py` (未迁移，见 `ldmdet-experiment/` 备份)
...
| 组件 | 位置 |
|:---|:---|
| 多步法调度器 | `ldmdet/diffusion/rectified_flow.py::RFDPMSolverMultistep` |
| 推理接入 | `ldmdet/core/head.py::predict()` `solver_type='dpm_solver_pp'` |
| 训练配置 | `configs/recipes/ldmdet_dpm_solver_pp.py` (6步), `ldmdet_dpm_solver_pp_s8.py` (8步) |
| 离线对比 | `projects/LDMDet/tools/compare_solver.py` (未迁移，见 `ldmdet-experiment/` 备份) |

### D.4 离线验证（关键结果）

> ⚠️ **暂时废弃**：以下离线验证与训练实验结果基于旧数据集 Chromosome20240904（mAP≈0.72-0.75，Heun 0.753、DPM-2 0.755 等），24obj 数据集上的结论已更新（A4 DPM-Solver++ 在 24obj 上 mAP=0.863）。下表的求解器对比数值不可直接引用。
