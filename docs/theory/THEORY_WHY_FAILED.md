...
> ⚠️ **暂时废弃**：以下第 5 条提到的 "LDMDet 基线 0.751 + Scale-Conditioned 负结果" 基于旧数据集 Chromosome20240904（mAP≈0.72-0.75），24obj 数据集上的结论已更新。上方的理论定理（Theorem 1-3）与信息论推导不受影响。
...
> ⚠️ **暂时废弃**：以下 Reflow 实验现象及梯度冲突测量（mAP 0.739、ρ=-0.104 等）基于旧数据集 Chromosome20240904（mAP≈0.72-0.75），24obj 数据集上的结论已更新。梯度冲突的机制假说（方向 D）仍可参考，但具体 mAP/ρ 数值不可引用。
...
> ⚠️ **暂时废弃**：以下 SOTA 路径负交互实验（mAP 0.740-0.752）基于旧数据集 Chromosome20240904（mAP≈0.72-0.75），24obj 数据集上的结论已更新。耦合-训练动力学冲突假说（方向 E）仍可参考，但具体 mAP 数值不可引用。
...
> ⚠️ **暂时废弃**：以下综合结论表中的实验依据（Reflow mAP 0.739、组合实验 0.740-0.752 等）基于旧数据集 Chromosome20240904（mAP≈0.72-0.75），24obj 数据集上的结论已更新。各方向的数学本质分析与可否修复建议作为机制假说仍可参考。
