...
### 阶段 6：DAEC 与表征-耦合联合对齐 (2026-05-28 ~ 至今)

| 实验 | mAP | 核心发现 |
| :--- | :--- | :--- |
| `daec_phase1_contrastive` | 0.738 | ✅ 跑通对比损失流程；❌ 出现特征空间坍缩 (Intra/Inter cos 同步上升)，对齐比仅 1.02。 |
| `daec_phase2_semantic` | 待测 | 规划中：引入语义感知 OT 代价 + 动态温度系数，打破信息截断。 |

**核心理论突破：打破“信息截断” (Information Cutoff)**
- **诊断发现**：`diagnose_daec.py` 证实，即使表征层强力对齐 (Self-Attn 参数正交化)，若 OT 耦合层依然只按几何距离分配，会导致表征层学习到错误的对齐方向，最终诱发特征空间整体坍缩。
- **DAEC Phase 2 逻辑**：耦合层必须“觉醒”语义。通过 no-grad forward 获取初步分类预测，将分类代价融入 Sinkhorn 矩阵，确保正样本分配在语义上是合理的。
- **对抗坍缩**：调低对比温度 $\tau$ 至 0.05，增强特征排斥力。
