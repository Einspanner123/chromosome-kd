...
def draw_rf():
    ...
    # RF Path (Straight)
    plt.plot([p_noise[0], p_data[0]], [p_noise[1], p_data[1]], 'r-', linewidth=3, label='Rectified Flow (Linear)')
    plt.arrow(0.65, 0.65, -0.1, -0.1, head_width=0.03, color='r')
    
    # Diffusion Path (Curved)
    t = np.linspace(0, 1, 100)
    curved_x = p_data[0] + (p_noise[0] - p_data[0]) * t + 0.15 * np.sin(np.pi * t)
    curved_y = p_data[1] + (p_noise[1] - p_data[1]) * t - 0.1 * np.sin(np.pi * t)
    plt.plot(curved_x, curved_y, 'b--', alpha=0.6, label='Traditional Diffusion (Curved)')
    
    plt.scatter(*p_noise, color='black', s=100, zorder=5)
    plt.text(p_noise[0]+0.02, p_noise[1], 'Noise (t=1)', fontsize=12, fontweight='bold')
    plt.scatter(*p_data, color='green', s=100, zorder=5)
    plt.text(p_data[0]-0.15, p_data[1]-0.05, 'Data (t=0)', fontsize=12, fontweight='bold', color='green')
...
def draw_heun():
    ...
    plt.plot([t_curr, t_next], [y_curr, y_euler], 'b--', marker='o', label='Euler Step (High Error)')
    
    # Heun step
    slope_next = -1.2 * t_next # Simplified: assuming we know the next slope
    y_heun = y_curr + 0.5 * (slope_curr + slope_next) * (t_next - t_curr)
    plt.plot([t_curr, t_next], [y_curr, y_heun], 'r-', marker='s', linewidth=2, label='Heun Step (2nd Order Correction)')
    
    plt.annotate('Predictor', xy=(t_next, y_euler), xytext=(t_next+0.05, y_euler+0.05), arrowprops=dict(arrowstyle='->'))
    plt.annotate('Corrector', xy=(t_next, y_heun), xytext=(t_next+0.05, y_heun-0.05), arrowprops=dict(arrowstyle='->'))
...
def draw_logit_coupling():
    ...
    ax1.add_patch(plt.Rectangle((0.2, 0.2), 0.3, 0.5, fill=False, edgecolor='blue', alpha=0.3, linestyle='--'))
    ax1.add_patch(plt.Rectangle((0.4, 0.3), 0.3, 0.5, fill=False, edgecolor='red', linewidth=2))
    ax1.annotate('', xy=(0.55, 0.55), xytext=(0.35, 0.45), arrowprops=dict(arrowstyle='->', color='red'))
    ...
    ax2.bar(x - 0.15, scores_t, 0.3, label='Score at t_curr', color='blue', alpha=0.3)
    ax2.bar(x + 0.15, scores_next, 0.3, label='Score at t_next', color='red')
    ax2.set_xticks(x); ax2.set_xticklabels(classes)
    ax2.set_ylabel("Confidence")
    ax2.legend()
    
    plt.suptitle("Logit-Velocity Coupling: Spatial-Semantic Alignment", fontsize=14, y=1.05)
...
def draw_shifted_schedule():
    ...
    for s, c, st in zip(shifts, colors, styles):
        ...
        if s == 3.0:
            ...
            plt.scatter(sample_t, sample_t_shifted, color='red', s=30, zorder=5)
    ...
    plt.annotate('More steps near Data (t=0)', xy=(0.2, 0.4), xytext=(0.4, 0.2),
