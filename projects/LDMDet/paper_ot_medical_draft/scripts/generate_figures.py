...
def fig1_overview(per_class):
    ...
    # Title
    ax.text(5, 5.6, "Diversity Over Efficiency: Sinkhorn Sampling for Dense Chromosome Detection",
            ha="center", va="center", fontsize=16, fontweight="bold")

    # ---- Row 1: The Problem ----
    # Box 1: OT intuition
    box1 = FancyBboxPatch((0.5, 3.8), 3.5, 1.4, boxstyle="round,pad=0.1",
                           facecolor="#FFF3E0", edgecolor="#E65100", linewidth=2)
    ax.add_patch(box1)
    ax.text(2.25, 4.8, "Common Belief", ha="center", fontsize=12, fontweight="bold", color="#E65100")
    ax.text(2.25, 4.4, "Optimal Transport coupling\nshortens transport paths &\nbenefits generation models",
            ha="center", fontsize=9.5, color="#333")

    # Arrow to Box 2
    ax.annotate("", xy=(5.5, 4.5), xytext=(4.0, 4.5),
                arrowprops=dict(arrowstyle="->", color="#555", lw=2))

    # Box 2: The Discovery
    box2 = FancyBboxPatch((5.5, 3.8), 3.5, 1.4, boxstyle="round,pad=0.1",
                           facecolor="#FFEBEE", edgecolor="#B71C1C", linewidth=2)
    ax.add_patch(box2)
    ax.text(7.25, 4.8, "Our Discovery", ha="center", fontsize=12, fontweight="bold", color="#B71C1C")
    ax.text(7.25, 4.35, f"OT harms dense detection:\nRandom mAP = {per_class.get('Random (AdaLN)', {}).get('mAP', 0.751):.3f}\nHard OT mAP = {per_class.get('Hard OT', {}).get('mAP', 0.735):.3f}",
            ha="center", fontsize=9.5, color="#333")

    # ---- Row 2: The Mechanism ----
    arrow_down = FancyArrowPatch((4.0, 3.8), (2.5, 3.0),
                                  arrowstyle="->", color="#555", lw=1.5,
                                  connectionstyle="arc3,rad=-0.2")
    ax.add_patch(arrow_down)

    box3 = FancyBboxPatch((0.5, 1.6), 4.0, 1.3, boxstyle="round,pad=0.1",
                           facecolor="#E3F2FD", edgecolor="#1565C0", linewidth=2)
    ax.add_patch(box3)
    ax.text(2.5, 2.55, "Mechanism 1: Diversity Collapse", ha="center", fontsize=11, fontweight="bold", color="#1565C0")
    ax.text(2.5, 2.15, "Conditional velocity entropy\nH(V|Z): 3.841 → 0.000 nats\nTotal variance: 5.169 → 2.401",
            ha="center", fontsize=9, color="#333")

    box4 = FancyBboxPatch((5.5, 1.6), 4.0, 1.3, boxstyle="round,pad=0.1",
                           facecolor="#E8F5E9", edgecolor="#2E7D32", linewidth=2)
    ax.add_patch(box4)
    ax.text(7.5, 2.55, "Mechanism 2: Argmax Bottleneck", ha="center", fontsize=11, fontweight="bold", color="#2E7D32")
    ax.text(7.5, 2.15, "Argmax destroys ε control\nD_eff frozen at 2.80 across ε\nStochastic restores D_eff to 5.31",
            ha="center", fontsize=9, color="#333")

    # ---- Row 3: The Solution ----
    ax.annotate("", xy=(7.25, 1.6), xytext=(4.75, 2.9),
                arrowprops=dict(arrowstyle="->", color="#555", lw=1.5,
                               connectionstyle="arc3,rad=0.2"))

    box5 = FancyBboxPatch((3, 0.2), 4.5, 0.9, boxstyle="round,pad=0.1",
                           facecolor="#C8E6C9", edgecolor="#1B5E20", linewidth=2)
    ax.add_patch(box5)
    ax.text(5.25, 0.9, "Stochastic Coupling", ha="center", fontsize=12, fontweight="bold", color="#1B5E20")
    ax.text(5.25, 0.5, f"Sample from Sinkhorn matrix → mAP = {per_class.get('Sinkhorn sample eps=5', {}).get('mAP', 0.750):.3f}\nPreserves diversity + transport structure",
            ha="center", fontsize=9.5, color="#333")
...
def fig2_coupling():
    ...
    titles = ["(a) Random Coupling", "(b) Hard OT", "(c) Sinkhorn + Argmax", "(d) Stochastic Coupling"]
    ...
    for ax, P, title in zip(axes, matrices, titles):
        ...
        ax.text(0.5, -0.3, f"Mean row entropy: {mean_entropy:.2f}",
                transform=ax.transAxes, ha="center", fontsize=8, color="#555")

    plt.colorbar(im, ax=axes, label="Assignment probability", shrink=0.6, pad=0.02)
    fig.tight_layout()
...
def fig3_epsilon():
    ...
    # rho and eta curves
    ax1.semilogx(eps_vals, rho, "o-", color=COLORS["random"], lw=2.5, markersize=8, label=r"$\rho(\varepsilon)$ Diversity Recovery")
    ax1.semilogx(eps_vals, eta, "s--", color=COLORS["hard_ot"], lw=2.5, markersize=8, label=r"$\eta(\varepsilon)$ Transport Efficiency")
    ...
    ax2.semilogx(mAP_eps, mAP_vals, "D-", color="#333", lw=2.5, markersize=10, label="mAP")
    ax2.set_ylabel("mAP", color="#333")
    ax2.set_ylim(0.730, 0.756)
    ax2.legend(loc="upper right", frameon=True)

    # Annotations
    ax1.annotate("OT-dominated\n($\\rho<0.95$)", xy=(0.05, 0.35), fontsize=10, color="#B71C1C", ha="center",
                bbox=dict(boxstyle="round,pad=0.3", facecolor="white", alpha=0.8))
    ax1.annotate("Sweet Spot\n($0.5\\leq\\varepsilon\\leq 5$)", xy=(1.5, 0.92), fontsize=10, color="#1B5E20", ha="center",
                bbox=dict(boxstyle="round,pad=0.3", facecolor="white", alpha=0.8))
    ax1.annotate("Bias-dominated\n($\\varepsilon>5$)", xy=(20, 1.05), fontsize=10, color="#E65100", ha="center",
                bbox=dict(boxstyle="round,pad=0.3", facecolor="white", alpha=0.8))
    ax2.annotate(f"mAP={0.750:.3f}", xy=(5, 0.750), xytext=(8, 0.754),
                fontsize=10, fontweight="bold", ha="center",
                arrowprops=dict(arrowstyle="->", color="#333"))

    ax1.set_title("Epsilon Sweep: Three-Regime Structure", fontweight="bold", pad=10)
    fig.tight_layout()
    ...
    for i, ax in enumerate(axes.flat):
        if i < len(img_paths):
            try:
                ...
                pass
        ax.set_title(f"Dataset A: 24-class chromosome\n(avg 46 objects/image)", fontsize=10, fontweight="bold")
        ax.axis("off")

    fig.suptitle("Dataset: Dense Multi-Instance Chromosome Detection", fontweight="bold", fontsize=14)
    fig.tight_layout()
...
def fig5_per_class_ap(per_class):
    ...
    random_ap = [data["Random (AdaLN)"]["per_class"].get(c, 0) for c in CLASS_NAMES]
    hardot_ap = [data["Hard OT"]["per_class"].get(c, 0) for c in CLASS_NAMES]
    stoch_ap = [data["Sinkhorn sample eps=5"]["per_class"].get(c, 0) for c in CLASS_NAMES]
    argmax_ap = [data.get("Sinkhorn argmax eps=5", {}).get("per_class", {}).get(c, 0) for c in CLASS_NAMES]
    ...
    bars1 = ax.bar(x - 1.5*width, random_ap, width, color=COLORS["random"], label="Random (0.751 mAP)", edgecolor="white", linewidth=0.5)
    bars2 = ax.bar(x - 0.5*width, hardot_ap, width, color=COLORS["hard_ot"], label="Hard OT (0.735 mAP)", edgecolor="white", linewidth=0.5)
    bars3 = ax.bar(x + 0.5*width, argmax_ap, width, color=COLORS["argmax"], label="Sinkhorn argmax ε=5 (0.744 mAP)", edgecolor="white", linewidth=0.5)
    bars4 = ax.bar(x + 1.5*width, stoch_ap, width, color=COLORS["stochastic"], label="Stochastic ε=5 (0.750 mAP)", edgecolor="white", linewidth=0.5)
    ...
    for g, positions in group_positions.items():
        ...
        ax.annotate(g, xy=(mid, 0.505), ha="center", fontsize=9, fontweight="bold", color="#666")

    # Arrow showing OT drops 22/24
    ax.annotate("Hard OT degrades\n22 out of 24 classes",
                xy=(10, 0.55), fontsize=11, fontweight="bold", color="#B71C1C",
                ha="center",
                bbox=dict(boxstyle="round,pad=0.3", facecolor="#FFEBEE", edgecolor="#B71C1C", alpha=0.9))

    ax.set_title("Per-Class AP: Coupling Strategy Comparison", fontweight="bold")
    fig.tight_layout()
...
def fig6_entropy():
    ...
    categories = ["H(V|Z)", "H(V|X_t)", "Total Var", "Between Var", "Within Var"]
    ...
    ax.bar(x - 1.5*width, random_vals, width, color=COLORS["random"], label="Random", edgecolor="white")
    ax.bar(x - 0.5*width, hardot_vals, width, color=COLORS["hard_ot"], label="Hard OT", edgecolor="white")
    ax.bar(x + 0.5*width, stoch1_vals, width, color=COLORS["argmax"], label="Stochastic ε=1", edgecolor="white")
    ax.bar(x + 1.5*width, stoch5_vals, width, color=COLORS["stochastic"], label="Stochastic ε=5", edgecolor="white")
...
def fig7_deff():
    ...
    if not deff or "results" not in deff:
        print("WARNING: deff_results.json not found. Run compute_deff.py first.")
        return
    ...
    ax.errorbar(eps_vals, deff_argmax, yerr=deff_argmax_std,
                fmt="s-", color=COLORS["hard_ot"], lw=2.5, markersize=8,
                capsize=4, label="Sinkhorn + Argmax")
    ax.errorbar(eps_vals, deff_stoch, yerr=deff_stoch_std,
                fmt="o-", color=COLORS["stochastic"], lw=2.5, markersize=8,
                capsize=4, label="Sinkhorn Sampling")

    ax.set_xscale("log")
    ax.set_xlabel(r"$\varepsilon$ (log scale)")
    ax.set_ylabel(r"Effective Match Count $D_{\mathrm{eff}}$")
    ax.set_title(r"$D_{\mathrm{eff}}$ vs $\varepsilon$: Argmax Destroys Diversity Control", fontweight="bold")
    ax.legend(loc="lower right", frameon=True)
    ax.grid(True, alpha=0.3, which="both")

    # Annotation
    ax.annotate("Argmax: D_eff remains\nconstrained across ε",
                xy=(5, deff_argmax[eps_vals == 5][0]),
                xytext=(15, deff_argmax[eps_vals == 5][0] - 5),
                fontsize=10, color="#B71C1C", ha="center",
                arrowprops=dict(arrowstyle="->", color="#B71C1C"),
                bbox=dict(boxstyle="round,pad=0.3", facecolor="white", alpha=0.8))
    ax.annotate("Stochastic: D_eff grows with ε",
                xy=(5, deff_stoch[eps_vals == 5][0]),
                xytext=(15, deff_stoch[eps_vals == 5][0] + 3),
                fontsize=10, color="#1B5E20", ha="center",
...
if __name__ == "__main__":
    ...
    if not per_class:
        print("ERROR: per_class_ap_results.json not found. Run per_class_ap.py first.")
        sys.exit(1)
