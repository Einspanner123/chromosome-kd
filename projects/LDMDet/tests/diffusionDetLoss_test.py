...
def test_losses():
    ...
    # 1. FocalLoss
    focal_loss = FocalLoss(use_sigmoid=True, alpha=0.25, gamma=2.0, loss_weight=2.0).to(
        device
    )
    ...
    target_box = torch.tensor([[15, 15, 55, 55]], dtype=torch.float32).to(device)
...
def test_criterion_forward():
    ...
    outputs = {
        ...
        "pred_boxes": torch.rand(bs, num_proposals, 4).to(device),  # 归一化 xyxy
        "aux_outputs": [
            {
                "pred_logits": torch.randn(bs, num_proposals, num_classes).to(device),
