...
def check_tensor(name, t):
    ...
    if has_nan or has_inf:
        ...
        print(
            f'  *** {name}: dtype={dtype}, shape={list(t.shape)}, NAN%={nan_pct:.1f}'
        )
        return True
    else:
        print(
            f'  {name}: dtype={dtype}, shape={list(t.shape)}, min={t.min().item():.4f}, max={t.max().item():.4f}, mean={t.mean().item():.4f}, std={t.std().item():.4f}'
        )
        return False
...
def verify():
    ...
    for i in range(4):
        print(
            f'  {i:<6} {feats_raw[i].max().item():>12.1f} {feats_raw[i].std().item():>12.1f} '
    ...
    for i in range(4):
        print(
            f'  {i:<6} {fpn_raw[i].max().item():>12.1f} {fpn_raw[i].std().item():>12.1f} '
