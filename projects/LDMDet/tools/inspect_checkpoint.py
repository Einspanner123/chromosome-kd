...
def inspect_ckpt(ckpt_path):
    ...
    reg_keys = [
        k for k in head_keys if 'reg' in k.lower() or 'bbox' in k.lower()
    ]
    token_keys = [
        k
        for k in head_keys
        if 'token' in k.lower()
        or 'pos_embed' in k.lower()
        or 'query' in k.lower()
    ]
    adaln_keys = [
        k
        for k in head_keys
        if 'adaln' in k.lower() or 'adaln_zero' in k.lower()
    ]
    ...
    for k, v in clean.items():
        if 'cls' in k.lower() and v.dim() == 1:  # bias
            print(
                f'  {k}: shape={v.shape} mean={v.mean():.6f} std={v.std():.6f} '
    ...
    for k in backbone_keys_pretrained:
        ...
        if v.dim() >= 1:
            if v.std() > 1e-6:
                backbone_updated += 1
    print(
        f'  Backbone params: {len(backbone_keys_pretrained)}, updated (>0 std): {backbone_updated}'
    )
    ...
    for k, v in clean.items():
        if 'query_embed' in k.lower() and v.dim() == 3:
            ...
            if off_diag > 0.99:
                print(
                    '  *** WARNING: 所有 query tokens 几乎完全相同! 模型无法区分不同 proposal。'
                )
            elif off_diag > 0.9:
                print('  ** 注意: query tokens 高度相似，可能导致预测框重复。')


if __name__ == '__main__':
    ckpt_path = str(
        REPO / 'work_dirs/ldmdet_dinov3_small_384_rope_shifted3/epoch_15.pth'
    )
