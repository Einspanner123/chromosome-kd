...
def analyze_errors(results_path, ann_file, iou_thr=0.5, score_thr=0.1):
    ...
    stats = {"Cls": 0, "Loc": 0, "Both": 0, "Dupe": 0, "Bkg": 0, "Miss": 0, "TP": 0}
    ...
    for i, img_id in enumerate(img_ids):
        ...
        gt_bboxes = (
            torch.tensor(gt_bboxes).float() if gt_bboxes else torch.zeros((0, 4))
        )
        gt_labels = (
    ...
    # 计算影响比例 (简单示意)
    total_errors = sum([v for k, v in stats.items() if k not in ["TP", "Miss"]])
    if total_errors > 0:
        print("\nError Distribution:")
