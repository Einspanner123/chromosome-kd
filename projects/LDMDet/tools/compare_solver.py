...
CONFIG = os.path.join(
    ...
    'configs', '_legacy', 'ldmdet_flowdet_adaln_ot_sinkhorn_sample_eps5.py',
)
...
def _parse_coco_metrics(text: str) -> dict:
    ...
    m = {}
    for pattern, key in [
        (r'AP.*IoU=0\.50:0\.95.*area=\s+all\s+\|\s+maxDets=100\s*\]\s*=\s*([\d.]+)', 'AP'),
        (r'AP.*IoU=0\.50\s+\|\s+area=\s+all\s+\|\s+maxDets=1000\s*\]\s*=\s*([\d.]+)', 'AP50'),
        (r'AP.*IoU=0\.75\s+\|\s+area=\s+all\s+\|\s+maxDets=1000\s*\]\s*=\s*([\d.]+)', 'AP75'),
        (r'AP.*IoU=0\.50:0\.95.*area=\s+small\s+\|\s+maxDets=1000\s*\]\s*=\s*([\d.]+)', 'APs'),
        (r'AP.*IoU=0\.50:0\.95.*area=medium\s+\|\s+maxDets=1000\s*\]\s*=\s*([\d.]+)', 'APm'),
        (r'AP.*IoU=0\.50:0\.95.*area=\s+large\s+\|\s+maxDets=1000\s*\]\s*=\s*([\d.]+)', 'APl'),
        (r'AR.*IoU=0\.50:0\.95.*area=\s+all\s+\|\s+maxDets=100\s*\]\s*=\s*([\d.]+)', 'AR'),
    ]:
...
def run_eval(config_path, ckpt_path, solver_type, sampling_timesteps, label):
    ...
    nfe = sampling_timesteps * 2 if solver_type == 'heun' else (sampling_timesteps + 1)
    ...
    with open(verbose_log, 'w') as vl:
        vl.write(f"=== {label}: {solver_type}, steps={sampling_timesteps}, NFE~{nfe} ===\n\n")
...
def main():
    ...
    lines = []
    lines.append(f"=== 求解器离线对比结果 === ({datetime.now().strftime('%Y-%m-%d %H:%M')})")
    ...
    if results:
        ...
        if heun_ap:
            ...
            for m in results[1:]:
                ap = m.get('AP', None)
                if ap:
                    lines.append(f"  {m.get('label','?'):<28} mAP = {ap:+.3f}")
