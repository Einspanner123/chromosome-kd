...
1.  **预测 (Predictor)**: 
    - 计算当前速度 $v_t = (x_t - x_{0,pred}) / t$。
    - 预估下一步位置：$x_{next, euler} = x_t + \Delta t \cdot v_t$。
2.  **修正 (Corrector)**:
    - 在新位置 $x_{next, euler}$ 再次调用模型，预测 $x_{0,pred\_next}$。
    - 计算新位置的速度 $v_{next}$。
    - **最终更新**: $x_{next} = x_t + \frac{\Delta t}{2} (v_t + v_{next})$。
3.  **集成 (Ensemble)**:
    - 为了稳健性，集成预测值取两次预测的均值：$x_{0,corr} = (x_{0,pred} + x_{0,pred\_next}) / 2$。
