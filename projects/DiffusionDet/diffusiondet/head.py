...
class DynamicDiffusionDetHead(nn.Module):
    ...
    def predict_by_feat(
        ...
        sampling_timesteps = cfg.get("sampling_timesteps", self.sampling_timesteps)
            ...
            for i, (scores_per_image, box_pred_per_image) in enumerate(
                ...
                if self.use_nms:
                    ...
                    if cfg.get("use_soft_nms", False):
                        if nms_cfg.get("type") != "soft_nms":
                            ...
                            if "iou_threshold" in cfg.nms:
                                nms_cfg["iou_threshold"] = cfg.nms["iou_threshold"]

                    det_bboxes, keep_idxs = batched_nms(
                        box_pred_per_image, scores_per_image, labels_per_image, nms_cfg
                    )
                    ...
                    if min_num > 0 and len(keep_idxs) < min_num:
                        ...
                        mask = torch.ones(
                            scores_per_image.shape[0], dtype=torch.bool, device=device
                        )
                        mask[keep_idxs] = False
                        remaining_scores = scores_per_image[mask]
                        remaining_indices = torch.nonzero(mask).squeeze(1)
                        if len(remaining_indices) > 0:
                            vals, sort_idx = remaining_scores.sort(descending=True)
                            to_add_indices = remaining_indices[sort_idx[:needed]]
                            keep_idxs = torch.cat([keep_idxs, to_add_indices])
                            added_boxes = box_pred_per_image[to_add_indices]
                            added_scores = scores_per_image[to_add_indices].unsqueeze(1)
                            added_dets = torch.cat([added_boxes, added_scores], dim=1)
...
class SingleDiffusionDetHead(nn.Module):
    ...
    def forward(self, features, bboxes, pro_features, pooler, time_emb):
        ...
        roi_features = roi_features.view(N * num_boxes, self.feat_channels, -1).permute(
            2, 0, 1
        )  # shape: (49, N*num_boxes, feat_channels)
