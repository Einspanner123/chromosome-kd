...
class DynamicDiffusionDetHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.time_mlp = nn.Sequential(
            ...
            nn.Linear(feat_channels, time_dim), nn.GELU(),  # 线性变换 + GELU激活
    ...
    def _init_weights(self):
        ...
        for p in self.parameters():
            if p.dim() > 1:
                nn.init.xavier_uniform_(p)  # 对于维度大于1的参数使用Xavier初始化
    ...
    def _build_diffusion(self):
        ...
        self.register_buffer('sqrt_alphas_cumprod', torch.sqrt(alphas_cumprod))  # sqrt(alpha_t_bar)
        self.register_buffer('sqrt_one_minus_alphas_cumprod', torch.sqrt(1. - alphas_cumprod))  # sqrt(1 - alpha_t_bar)
        self.register_buffer('log_one_minus_alphas_cumprod', torch.log(1. - alphas_cumprod))  # log(1 - alpha_t_bar)
        self.register_buffer('sqrt_recip_alphas_cumprod', torch.sqrt(1. / alphas_cumprod))  # sqrt(1 / alpha_t_bar)
        self.register_buffer('sqrt_recipm1_alphas_cumprod', torch.sqrt(1. / alphas_cumprod - 1))  # sqrt(1 / alpha_t_bar - 1)
 
        # 后验分布q(x_{t-1} | x_t, x_0)所需计算
        # 计算后验方差: beta_t * (1 - alpha_{t-1}_bar) / (1 - alpha_t_bar)
        posterior_variance = betas * (1. - alphas_cumprod_prev) / (1. - alphas_cumprod)
        self.register_buffer('posterior_variance', posterior_variance)  # 后验方差
    ...
    def forward(self, features, init_bboxes, init_t, init_features=None):
        ...
        if init_features is not None:
            # 扩展初始特征以匹配批次大小
            init_features = init_features[None].repeat(1, bs, 1)  # shape: [1, batch_size, channels]
    ...
    def q_sample(self, x_start, time, noise=None):
        ...
        sqrt_alphas_cumprod_t = extract(
            self.sqrt_alphas_cumprod, time, x_start_shape)  # shape: [1, 1, 1]
        sqrt_one_minus_alphas_cumprod_t = extract(
            self.sqrt_one_minus_alphas_cumprod, time, x_start_shape)  # shape: [1, 1, 1]
    ...
    def do_results_post_process(results_list, cfg, batch_img_metas=None):
        ...
        for results, img_meta in zip(results_list, batch_img_metas):
            ...
            scale_factor = [1 / s for s in img_meta['scale_factor']]  # 缩放因子
            results.bboxes = scale_boxes(results.bboxes, scale_factor)  # 缩放边界框
            # 裁剪边界框到图像范围内
            h, w = img_meta['ori_shape']
            results.bboxes[:, 0::2] = results.bboxes[:, 0::2].clamp(
                min=0, max=w)  # x坐标限制在[0,w]
            results.bboxes[:, 1::2] = results.bboxes[:, 1::2].clamp(
                min=0, max=h)  # y坐标限制在[0,h]

            # 过滤小尺寸边界框
            if cfg.get('min_bbox_size', 0) >= 0:
                w, h = get_box_wh(results.bboxes)  # 获取框的宽高
                valid_mask = (w > cfg.min_bbox_size) & (h > cfg.min_bbox_size)  # 有效框掩码
    ...
    def prepare_testing_targets(self, batch_img_metas, device):
        ...
        for img_meta in batch_img_metas:
            ...
            image_size = torch.tensor([w, h, w, h],
                                      dtype=torch.float32,
                                      device=device)  # shape: [4]
            # 生成随机噪声框
            noise_bboxes_raw = torch.randn((self.num_proposals, 4),
                                           device=device)  # shape: [num_proposals, 4]
            # 处理噪声框
            noise_bboxes = torch.clamp(
                noise_bboxes_raw, min=-1 * self.snr_scale, max=self.snr_scale)  # 限制范围
            noise_bboxes = ((noise_bboxes / self.snr_scale) + 1) / 2  # 转换到[0,1]
            noise_bboxes = bbox_cxcywh_to_xyxy(noise_bboxes)  # 转换为xyxy格式
            noise_bboxes = noise_bboxes * image_size  # 缩放到绝对坐标
        ...
        # 堆叠为批次张量
        batch_noise_bboxes = torch.stack(noise_bboxes_list)  # shape: [batch_size, num_proposals, 4]
        batch_image_size = torch.cat(image_size_list)  # shape: [batch_size, 4]
        batch_noise_bboxes_raw = torch.stack(noise_bboxes_raw_list)  # shape: [batch_size, num_proposals, 4]
        ...
        if self.use_focal_loss or self.use_fed_loss:
            ...
            labels = torch.arange(
                ...
                device=device).unsqueeze(0).repeat(self.num_proposals,
                                                   1).flatten(0, 1)  # shape: [num_proposals * num_classes]
            ...
            for i, (scores_per_image,
                ...
                scores_per_image, topk_indices = scores_per_image.flatten(
                    0, 1).topk(
                        self.num_proposals, sorted=False)  # 选择前num_proposals个预测
                labels_per_image = labels[topk_indices]  # 对应的标签
                # 重复边界框预测以匹配类别
                box_pred_per_image = box_pred_per_image.view(-1, 1, 4).repeat(
                    1, self.num_classes, 1).view(-1, 4)  # shape: [num_proposals*num_classes, 4]
                box_pred_per_image = box_pred_per_image[topk_indices]  # 选择对应预测
            ...
            scores, labels = F.softmax(box_cls, dim=-1)[:, :, :-1].max(-1)  # softmax激活并获取最大得分和标签
...
class SingleDiffusionDetHead(nn.Module):
    ...
    def __init__(
        ...
        dynamic_conv=dict(
            dynamic_dim=64, dynamic_num=2)  # 动态卷积配置
    ) -> None:
        super().__init__()
        self.feat_channels = feat_channels  # 特征通道数
        ...
        self.block_time_mlp = nn.Sequential(
            nn.SiLU(),  # SiLU激活
            nn.Linear(feat_channels * 4, feat_channels * 2))  # 线性变换

        # 分类模块
        cls_module = list()
        for _ in range(num_cls_convs):
            cls_module.append(nn.Linear(feat_channels, feat_channels, False))  # 线性层（无偏置）
        ...
        for _ in range(num_reg_convs):
            reg_module.append(nn.Linear(feat_channels, feat_channels, False))  # 线性层（无偏置）
    ...
    def forward(self, features, bboxes, pro_features, pooler, time_emb):
        ...
        roi_features = roi_features.view(
            N * num_boxes, self.feat_channels, -1).permute(2, 0, 1)  # shape: (49, N*num_boxes, feat_channels)

        # 自注意力
        pro_features = pro_features.view(
            N, num_boxes, self.feat_channels).permute(1, 0, 2)  # shape: (num_boxes, N, feat_channels)
        # 执行自注意力: Query=Key=Value=pro_features
        pro_features2 = self.self_attn(
            pro_features, pro_features, value=pro_features)[0]  # shape: (num_boxes, N, feat_channels)
        # 残差连接和归一化
        ...
        pro_features = pro_features.view(
            ...
            self.feat_channels).permute(1, 0,
                                        2).reshape(1, N * num_boxes,
                                                   self.feat_channels)  # shape: (1, N*num_boxes, feat_channels)
        # 动态卷积交互
        pro_features2 = self.inst_interact(pro_features, roi_features)
        # 残差连接和归一化
        pro_features = pro_features + self.dropout2(pro_features2)
        obj_features = self.norm2(pro_features)  # shape: (1, N*num_boxes, feat_channels)

        # 对象特征处理（前馈网络）
        obj_features2 = self.linear2(
            self.dropout(self.activation(self.linear1(obj_features))))  # 前馈网络
        obj_features = obj_features + self.dropout3(obj_features2)  # 残差连接
        obj_features = self.norm3(obj_features)  # 归一化

        # 时间嵌入条件化
        fc_feature = obj_features.transpose(0, 1).reshape(N * num_boxes, -1)  # shape: (N*num_boxes, feat_channels)

        scale_shift = self.block_time_mlp(time_emb)  # 时间嵌入MLP
        scale_shift = torch.repeat_interleave(scale_shift, num_boxes, dim=0)  # 扩展到每个框
        ...
        # 预测分类logits和边界框偏移
        class_logits = self.class_logits(cls_feature)  # shape: (N*num_boxes, num_classes)
        bboxes_deltas = self.bboxes_delta(reg_feature)  # shape: (N*num_boxes, 4)
        pred_bboxes = self.apply_deltas(bboxes_deltas, bboxes.view(-1, 4))  # 应用偏移到边界框
...
class DynamicConv(nn.Module):
    ...
    def __init__(self,
                 ...
                 pooler_resolution: int = 7) -> None:  # 池化分辨率
        super().__init__()
    ...
    def forward(self, pro_features: Tensor, roi_features: Tensor) -> Tensor:
        ...
        parameters = self.dynamic_layer(pro_features).permute(1, 0, 2)  # shape: (N*num_boxes, 1, dynamic_num*num_params)

        # 分割参数为两个卷积层的参数
        param1 = parameters[:, :, :self.num_params].view(-1, self.feat_channels, self.dynamic_dim)  # shape: (N*num_boxes, feat_channels, dynamic_dim)
        param2 = parameters[:, :, self.num_params:].view(-1, self.dynamic_dim, self.feat_channels)  # shape: (N*num_boxes, dynamic_dim, feat_channels)

        # 执行动态卷积: 第一层
        features = torch.bmm(features, param1)  # shape: (N*num_boxes, feat_channels, dynamic_dim)
        features = self.norm1(features)  # 归一化
        features = self.activation(features)  # 激活

        # 执行动态卷积: 第二层
        features = torch.bmm(features, param2)  # shape: (N*num_boxes, feat_channels, feat_channels)
