...
def cosine_beta_schedule(timesteps, s=0.008):
    ...
    steps = timesteps + 1
    x = torch.linspace(
        0, timesteps, steps, dtype=torch.float64)  # shape: [timesteps+1]
    # 计算累积alpha值,使用余弦函数生成递减序列
    alphas_cumprod = (torch.cos(
        ((x / timesteps) + s) / (1 + s) * math.pi * 0.5)**2)  # 取值范围[0, 1]
...
def extract(a, t, x_shape):
    ...
    return out.reshape(
        batch_size,
        *((1, ) *
          (len(x_shape) - 1)))  # shape: [bs, 1, 1, 1, ..., 1(输入的维度数-1)]
...
class DynamicDiffusionDetHead(nn.Module):
    ...
    def _init_weights(self):
        ...
        for p in self.parameters():
            ...
            if self.use_focal_loss or self.use_fed_loss:
                if p.dim() > 0 and (p.shape[-1] == self.num_classes
    ...
    def _build_diffusion(self):
        ...
        self.register_buffer('alphas_cumprod_prev',
                             alphas_cumprod_prev)  # 前一个累积alpha值
        ...
        self.register_buffer(
            ...
            torch.sqrt(1.0 - alphas_cumprod))  # sqrt(1 - alpha_t_bar)
        self.register_buffer('log_one_minus_alphas_cumprod',
                             torch.log(1.0 -
                                       alphas_cumprod))  # log(1 - alpha_t_bar)
        self.register_buffer(
            'sqrt_recip_alphas_cumprod',
            torch.sqrt(1.0 / alphas_cumprod))  # sqrt(1 / alpha_t_bar)
        self.register_buffer('sqrt_recipm1_alphas_cumprod',
                             torch.sqrt(1.0 / alphas_cumprod -
                                        1))  # sqrt(1 / alpha_t_bar - 1)
    ...
    def forward(self, features, init_bboxes, init_t, init_features=None):
        ...
        if init_features is not None:
            ...
            init_features = init_features[None].repeat(
                1, bs, 1)  # shape: [1, batch_size, channels]
    ...
    def loss(self, x: Tuple[Tensor], batch_data_samples: SampleList) -> dict:
        ...
        output = {
            'pred_logits': pred_logits[
                -1],  # 最后一层输出,shape: [batch_size, num_proposals, num_classes]
            'pred_boxes':
            pred_bboxes[-1],  # 最后一层输出,shape: [batch_size, num_proposals, 4]
        }
        # 如果使用深度监督,添加辅助输出
        if self.deep_supervision:
            output['aux_outputs'] = [{
    ...
    def prepare_training_targets(self, batch_data_samples):
        ...
        for data_sample in batch_data_samples:
            ...
            image_size = gt_bboxes.new_tensor([w, h, w,
                                               h])  # 图像大小张量,shape: [4]
            ...
            norm_gt_bboxes_cxcywh = bbox_xyxy_to_cxcywh(
                norm_gt_bboxes)  # shape: [num_gts, 4]
            # 准备扩散训练目标（添加噪声）
    ...
    def prepare_diffusion(self, gt_boxes, image_size):
        ...
        # 随机采样一个时间步
        time = torch.randint(
            0, self.timesteps, (1, ), dtype=torch.long,
            device=device)  # shape: [1]
        # 生成随机噪声
        noise = torch.randn(
            self.num_proposals, 4, device=device)  # shape: [num_proposals, 4]
        ...
        if num_gt < self.num_proposals:
            ...
            box_placeholder = (
                ...
                6.0 + 0.5)  # shape: [num_proposals-num_gt, 4]
            box_placeholder[:, 2:] = torch.clip(
                box_placeholder[:, 2:], min=1e-4)  # 保证宽度和高度为正
            x_start = torch.cat((gt_boxes, box_placeholder),
    ...
    def q_sample(self, x_start, time, noise=None):
        ...
        sqrt_alphas_cumprod_t = extract(self.sqrt_alphas_cumprod, time,
                                        x_start_shape)  # shape: [1, 1, 1]
        sqrt_one_minus_alphas_cumprod_t = extract(
            self.sqrt_one_minus_alphas_cumprod, time,
            x_start_shape)  # shape: [1, 1, 1]
    ...
    def predict_by_feat(
        ...
        sampling_timesteps = cfg.get('sampling_timesteps',
        ...
        for time, time_next in time_pairs:
            ...
            x_start = (x_start * 2 -
                       1.0) * self.snr_scale  # 缩放到[-snr_scale, snr_scale]
            x_start = torch.clamp(
            ...
            if self.box_renewal:  # 如果使用框更新
                ...
                for img_id in range(batch_size):
                    ...
                    score_per_image = torch.sigmoid(
                        score_per_image)  # sigmoid激活
                    value, _ = torch.max(
                        score_per_image, -1, keepdim=False)  # 每个框的最大类别得分
                    ...
                    pred_noise_list.append(
                        pred_noise[img_id, keep_idx, :])  # 保留框对应的预测噪声
                    x_start_list.append(x_start[img_id,
                                                keep_idx, :])  # 保留框对应的去噪结果
                    noise_bboxes_list.append(
                        batch_noise_bboxes[img_id, keep_idx, :])  # 保留的噪声框
            ...
            sigma = (self.ddim_sampling_eta *
                     ((1 - alpha / alpha_next) * (1 - alpha_next) /
            ...
            for idx in range(batch_size):
                ...
                #                         sigma * noise
                noise_bboxes = (
                    x_start * alpha_next.sqrt() + c * pred_noise +
                    sigma * noise)
                ...
                if self.box_renewal:  # 如果使用框更新
                    ...
                    noise_bboxes = (
                        (noise_bboxes / self.snr_scale) + 1) / 2  # 转换到[0,1]
                    noise_bboxes = bbox_cxcywh_to_xyxy(
                        noise_bboxes)  # 转换为xyxy格式
    ...
    def prepare_testing_targets(self, batch_img_metas, device):
        ...
        for img_meta in batch_img_metas:
            ...
            # 处理噪声框
            noise_bboxes = torch.clamp(
                noise_bboxes_raw, min=-1 * self.snr_scale,
                max=self.snr_scale)  # 限制范围
            noise_bboxes = (
                (noise_bboxes / self.snr_scale) + 1) / 2  # 转换到[0,1]
        ...
        batch_noise_bboxes = torch.stack(
            noise_bboxes_list)  # shape: [batch_size, num_proposals, 4]
        batch_image_size = torch.cat(image_size_list)  # shape: [batch_size, 4]
        batch_noise_bboxes_raw = torch.stack(
            noise_bboxes_raw_list)  # shape: [batch_size, num_proposals, 4]
        return (
            time_pairs,
            batch_noise_bboxes,
            batch_noise_bboxes_raw,
        ...
        if self.use_focal_loss or self.use_fed_loss:
            ...
            labels = (torch.arange(self.num_classes,
                                   device=device).unsqueeze(0).repeat(
                                       self.num_proposals, 1).flatten(0, 1)
                    ...
                    if min_num > 0 and len(keep_idxs) < min_num:
                        ...
                        if len(remaining_indices) > 0:
                            ...
                            added_scores = scores_per_image[
                                to_add_indices].unsqueeze(1)
                            added_dets = torch.cat([added_boxes, added_scores],
                                                   dim=1)
            ...
            scores, labels = F.softmax(
                box_cls, dim=-1)[:, :, :-1].max(-1)  # softmax激活并获取最大得分和标签
...
class SingleDiffusionDetHead(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(num_cls_convs):
            cls_module.append(nn.Linear(feat_channels, feat_channels,
                                        False))  # 线性层（无偏置）
        ...
        for _ in range(num_reg_convs):
            reg_module.append(nn.Linear(feat_channels, feat_channels,
                                        False))  # 线性层（无偏置）
    ...
    def forward(self, features, bboxes, pro_features, pooler, time_emb):
        ...
        if pro_features is None:
            ...
            pro_features = roi_features.view(
                ...
                -1).mean(-1)  # shape: (N, num_boxes, feat_channels)

        # 调整ROI特征形状以适应注意力机制
        roi_features = roi_features.view(
            N * num_boxes, self.feat_channels,
            -1).permute(2, 0, 1)  # shape: (49, N*num_boxes, feat_channels)

        # 自注意力
        pro_features = pro_features.view(
            N, num_boxes, self.feat_channels).permute(
                1, 0, 2)  # shape: (num_boxes, N, feat_channels)
        # 执行自注意力: Query=Key=Value=pro_features
        pro_features2 = self.self_attn(
            pro_features, pro_features,
            value=pro_features)[0]  # shape: (num_boxes, N, feat_channels)
        # 残差连接和归一化
        ...
        obj_features = self.norm2(
            pro_features)  # shape: (1, N*num_boxes, feat_channels)
        ...
        class_logits = self.class_logits(
            cls_feature)  # shape: (N*num_boxes, num_classes)
        bboxes_deltas = self.bboxes_delta(
            reg_feature)  # shape: (N*num_boxes, 4)
        pred_bboxes = self.apply_deltas(bboxes_deltas,
                                        bboxes.view(-1, 4))  # 应用偏移到边界框
...
class DynamicConv(nn.Module):
    ...
    def forward(self, pro_features: Tensor, roi_features: Tensor) -> Tensor:
        ...
        features = roi_features.permute(
            1, 0, 2)  # shape: (N*num_boxes, 49, feat_channels)
        parameters = self.dynamic_layer(pro_features).permute(
            1, 0, 2)  # shape: (N*num_boxes, 1, dynamic_num*num_params)
        ...
        features = torch.bmm(
            features,
            param1)  # shape: (N*num_boxes, feat_channels, dynamic_dim)
        ...
        features = torch.bmm(
            features,
            param2)  # shape: (N*num_boxes, feat_channels, feat_channels)
        features = self.norm2(features)  # 归一化
        features = self.activation(features)  # 激活

        # 展平并通过输出层
        features = features.flatten(
            1)  # 展平,shape: (N*num_boxes, dynamic_dim*feat_channels)
