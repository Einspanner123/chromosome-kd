"""
ChromosomeEncoder Denver 组分类能力基准验证

验证 encoder 能否区分 8 个 Denver 组 (比 24 类细粒度分类容易 3-5 倍)。
如果 8 组 acc > 60% → Denver 组级流匹配可行
如果 < 40% → 需要用纯 matching-based 方法
"""
...
def main():
    ...
    # 预计算 slot → group label (tensor, 46)
    ...
    classifier = nn.Sequential(
        ...
        nn.GELU(),
        nn.Linear(args.d_model, NUM_GROUPS),
    ).to(device)
    ...
    print(f"\nTraining {args.epochs} epochs for {NUM_GROUPS}-way Denver group classification...")
    print(f"Groups: {GROUP_NAMES}")
    print(f"Group distribution (per image):")
    for gi, gname in enumerate(GROUP_NAMES):
        count = (slot_to_group == gi).sum().item()
        print(f"  Group {gname}: {count} slots")
    best_acc = 0.0
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        with torch.no_grad():
            for batch in valid_loader:
                ...
                labels_v = slot_to_group.unsqueeze(0).expand(B_v, -1).reshape(-1).to(device)
    ...
    print(f"\n=== Result ===")
    print(f"Best val accuracy ({NUM_GROUPS}-way Denver groups): {best_acc:.1f}%")
    if best_acc > 60:
        print("✓ PASS — Denver group classification is viable for KaryoFlow")
    elif best_acc > 40:
        print("△ MARGINAL — Group features are weak, consider larger crop_size or backbone")
    else:
        print("✗ FAIL — Even group-level classification fails, use pure matching-based approach")
