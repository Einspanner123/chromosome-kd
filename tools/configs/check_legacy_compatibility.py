...
def strip_nonsemantic(value):
    ...
    if isinstance(value, dict):
        ...
        for key, child in value.items():
            if key == 'palette' or (
                key == 'backend_args' and child in (None, [], {})
            ):
                continue
...
def recursive_diff(left, right, prefix='') -> list[dict]:
    if isinstance(left, dict) and isinstance(right, dict):
        ...
        for key in sorted(set(left) | set(right)):
            ...
            if key not in left:
                output.append(
                    dict(path=path, legacy='__MISSING__', target=right[key])
                )
            elif key not in right:
                output.append(
                    dict(path=path, legacy=left[key], target='__MISSING__')
                )
            else:
    ...
    if isinstance(left, list) and isinstance(right, list):
        ...
        return output
    return (
        [] if left == right else [dict(path=prefix, legacy=left, target=right)]
    )
...
def leaf_dataset(cfg: Config, split: str):
    ...
    while (
        isinstance(dataset, (dict, ConfigDict))
        and 'dataset' in dataset
        and (
            'ann_file' not in dataset
            or 'data_root' not in dataset
...
def dataset_record(
    cfg: Config, split: str, annotation_override: Path | None = None
) -> dict:
    ...
    return dict(
        path=shown(path),
        configured_path=shown(configured_path),
        ...
        configured_matches_annotation=(
            configured == [x[1] for x in categories]
        ),
...
def verify_summary(
    ...
    checks = {
        'config_sha256': (
            protocol.get('config_source_sha256'),
        ...
        'annotation_sha256': (
            protocol.get('annotation_sha256'),
...
def checkpoint_lineage(parent_path: Path, child_path: Path) -> dict:
    ...
    return dict(
        ...
        final_quality_only=(
            not changed
            and not parent_only
            and len(child_only) == 5
            and all(
                '.head_series.5.quality_head.' in f'.{name}'
        ...
        parent_meta_seed_candidates=sorted(
            {
                item['value']
                for item in seed_candidates(parent_raw.get('meta', {}))
            }
        ),
    )
...
def main() -> int:
    ...
    parent_path = (
        resolve_path(args.parent_checkpoint)
    ...
    if absent:
        report = dict(
            ...
            report_id='legacy-compat-missing-'
            + hashlib.sha256(
                json.dumps(absent, sort_keys=True).encode()
            ).hexdigest()[:12],
    ...
    target_data = {
        split: dataset_record(target, split)
    ...
    for split in ('train', 'val', 'test'):
        ...
        if (
            legacy_data[split]['effective_categories']
            != target_data[split]['effective_categories']
        ):
            dataset_issues.append(
                f'{split} effective category mapping mismatch'
            )

    checkpoint = checkpoint_record(checkpoint_path, target)
    parent_required = target.experiment.get(
        'parent_checkpoint_required', False
    )
    lineage = (
        checkpoint_lineage(parent_path, checkpoint_path)
        if parent_path is not None
        else None
    )
    result = verify_summary(
        ...
        legacy_path,
        checkpoint_path,
        Path(target_data['test']['path'])
        if Path(target_data['test']['path']).is_absolute()
    ...
    checkpoint_seeds = sorted(
        {x['value'] for x in checkpoint['meta_seed_candidates']}
    )
    ...
    if parent_required:
        ...
        parent_evidence = [
            value
            for value in [summary_seed, *parent_seeds]
            if isinstance(value, int)
        ]
        child_evidence = checkpoint_seeds + log_seeds
        parent_consistent = (
            bool(parent_evidence) and len(set(parent_evidence)) == 1
        )
        child_consistent = (
            bool(child_evidence) and len(set(child_evidence)) == 1
        )
        ...
        authoritative = [
            value
            for value in [summary_seed, *checkpoint_seeds, *log_seeds]
            if isinstance(value, int)
        ]
        ...
        child_seed = None
    seeds = dict(
        directory_labels=directory_seed(
            [legacy_path, checkpoint_path, log_path]
        ),
    ...
    if parent_required and parent_path is None:
        hard_issues.append(
            'paired child is missing parent checkpoint evidence'
        )
    ...
    for difference in differences:
        ...
        elif path.startswith('data.test_evaluator.') or path.startswith(
            'data.test_dataloader.dataset.ann_file'
        ):
    ...
    scientific_differences = [
        d for d in differences if d['severity'] == 'scientific'
    ]
    ...
    report = dict(
        schema_version=1,
        generated_at=result['protocol'].get('completed_at')
        or json.loads(summary_path.read_text()).get('completed_at'),
        status=status,
        ...
        legacy=dict(
            ...
            training_log=dict(
                path=shown(log_path), sha256=sha256_file(log_path)
            ),
        ),
        seeds=seeds,
        dataset=dict(
