...
def main() -> int:
    ...
    for run in runs:
        ...
        records.append(dict(
            run_id=run['run_id'], report_path=run['output'],
            report_sha256=digest(path), report_id=report['report_id'],
    ...
    output = (ROOT / args.index_output if args.index_output else
              ROOT / 'tools/experiment_db/compatibility_reports' /
              f'{suite["suite_id"]}_index.json')
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(index, indent=2, sort_keys=True) + '\n')
    print(json.dumps(dict(status='PASS', reports=len(records),
                          index=display(output),
                          status_counts=index['status_counts']), indent=2))
    return 0
