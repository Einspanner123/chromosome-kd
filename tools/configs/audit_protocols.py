...
def main() -> int:
    ...
    for path in sorted(ABLATIONS.glob('*.yaml')):
        ...
        if (
            data['split'] == 'test'
            and data['selection'].get('test_tuned') is not False
        ):
            errors.append(
                f'{path.name}: test protocol must declare test_tuned=false'
            )
        base = data['base']
        ...
        if checkpoints:
            ...
            if len(set(seeds)) != len(seeds) or len(set(hashes)) != len(
                hashes
            ):
                errors.append(
                    f'{path.name}: checkpoint replication is not independent'
                )
    if errors:
