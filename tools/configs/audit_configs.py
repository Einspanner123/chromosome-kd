#!/usr/bin/env python3
"""Audit canonical sources, matrices, resolved configs, and model invariants."""
...
def depth(path: Path, seen: tuple[Path, ...] = ()) -> int:
    ...
    parents = bases(path)
    return (
        0 if not parents else 1 + max(depth(p, (*seen, path)) for p in parents)
    )
...
def main() -> int:
    ...
    errors = []
    files = sorted(CONFIG_ROOT.rglob('*.py'))
    sources = files + sorted((CONFIG_ROOT / 'matrices').glob('*.yaml'))
    for path in sources:
        if FORBIDDEN.search(path.read_text()):
            errors.append(
                f'{path.relative_to(ROOT)}: forbidden path/host/credential'
            )
    for path in files:
        try:
            ...
            if inherited > 3:
                errors.append(
                    f'{path.relative_to(ROOT)}: inheritance depth {inherited} > 3'
                )
            for parent in bases(path):
                if not parent.exists():
                    errors.append(
                        f'{path.relative_to(ROOT)}: missing parent {parent}'
                    )
                elif (
                    CONFIG_ROOT not in parent.parents and parent != CONFIG_ROOT
                ):
                    errors.append(
                        f'{path.relative_to(ROOT)}: inherits outside canonical config root'
                    )
        except Exception as exc:
            errors.append(f'{path.relative_to(ROOT)}: {exc}')

    if (CONFIG_ROOT / 'recipes').exists():
        errors.append('legacy recipes directory must not exist')
    methods = sorted((CONFIG_ROOT / 'methods').glob('*.py'))
    matrices = sorted((CONFIG_ROOT / 'matrices').glob('*.yaml'))
    for method_path in methods:
        try:
            ...
            errors.append(
                f'{method_path.relative_to(ROOT)}: method load failed: {exc}'
            )
    config_ids = set()
    resolved = []
    for matrix_path in matrices:
        try:
            ...
            if unknown:
                errors.append(
                    f'{matrix_path.relative_to(ROOT)}: unknown methods {unknown}'
                )
            for method_name in matrix['methods']:
                ...
                if model_type not in allowed_models:
                    errors.append(
                        f'{cid}: unsupported model type {model_type}'
                    )
                if cfg.test_evaluator.get('format_only', False):
                    errors.append(
                        f'{cid}: test evaluator must compute metrics'
                    )
                ...
                if 'lqcr' in meta.method_id:
                    if model_type != 'LDMDet':
                        ...
                        resolve_config(
                            matrix_path, name, matrix['training_seeds'][0]
                        )[0].experiment.method_id
                        for name in matrix['methods']
                    ]:
                        errors.append(
                            f'{cid}: parent method absent from matrix'
                        )
                    if meta.pairing != 'same_training_seed':
                        errors.append(f'{cid}: invalid parent pairing')
                    if (
                        head.quality_calibration_mode != 'final_only'
                ...
                if meta.role == 'paired_head_distillation':
                    ...
                    if (
                        meta.get('parent_checkpoint_binding')
                        != 'teacher_checkpoint'
                    ):
                        errors.append(
            ...
            errors.append(
                f'{matrix_path.relative_to(ROOT)}: resolve failed: {exc}'
            )
        ...
        for cid, cfg in resolved:
            with tempfile.NamedTemporaryFile(suffix='.py') as stream:
                ...
                if scientific_hash(loaded) != scientific_hash(cfg):
                    errors.append(
                        f'{cid}: resolved config round-trip mismatch'
                    )
                del loaded
                gc.collect()
    ...
    print(
        f'sources={len(sources)} methods={len(methods)} '
