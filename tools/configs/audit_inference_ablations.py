...
def git_blob_exists(source: dict) -> bool:
    ...
    return (
        ...
        and resolved.stdout.strip() == source['blob_sha1']
    )
...
def audit(path: Path) -> list[str]:
    ...
    if (
        ...
        or selection.get('test_tuned') is not False
    ):
    ...
    if not legacy_config.is_file() and not git_blob_exists(
        base['legacy_source']
    ):
        errors.append(
            'legacy config is absent and archived Git blob does not resolve'
        )
    ...
    if evidence.get('replay_status') == 'complete':
        ...
        if (
            evidence['aggregate_group'] in ('solver', 'topk_renewal')
            and len(artifact_fields) != 2
        ):
            errors.append(
                'complete replay status requires raw archive artifacts'
            )
    ...
    for row in results:
        ...
        if (
            protocol['split'] != data['split']
            or protocol['seed'] != data['inference_seed']
        ):
