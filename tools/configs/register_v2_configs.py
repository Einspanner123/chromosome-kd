#!/usr/bin/env python3
"""Register v2 matrix-method combinations without rewriting run records."""
...
CATALOG = V2 / 'manifests/experiment_catalog.json'
...
def main() -> int:
    ...
    referenced = conn.execute(
        """SELECT COUNT(*) FROM experiment
           WHERE config_path LIKE 'experiments/configs/v2/recipes/%'""").fetchone()[0]
    if referenced:
        raise RuntimeError('legacy v2 recipe rows are already referenced by runs')
    conn.execute(
        "DELETE FROM config WHERE config_path LIKE 'experiments/configs/v2/recipes/%'")

    for matrix_path in sorted((V2 / 'matrices').glob('*.yaml')):
