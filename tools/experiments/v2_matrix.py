...
RUNTIME = V2 / '_base_/runtime/default.py'
METHODS = V2 / 'methods'

SCIENTIFIC_KEYS = (
    'dataset_id', 'dataset_manifest_sha256',
    ...
    'test_evaluator', 'optim_wrapper', 'param_scheduler',
    'train_cfg', 'val_cfg', 'test_cfg', 'custom_hooks',
)
...
def _deep_merge(base, update):
    ...
    for key, value in update.items():
        output[key] = _deep_merge(output[key], value) if key in output else value
    return output
...
def load_matrix(path: str | Path) -> tuple[Path, dict]:
    ...
    required = {'schema_version', 'matrix_id', 'dataset_alias',
                'dataset_config', 'tracker_project', 'training_seeds', 'methods'}
...
def resolve_config(matrix_path: str | Path, method_name: str,
                   seed: int = 42, parent_checkpoint: str | None = None):
    matrix_path, matrix = load_matrix(matrix_path)
    if method_name not in matrix['methods']:
        raise ValueError(f'{method_name} is not enabled by {matrix["matrix_id"]}')
    if seed not in matrix['training_seeds']:
        raise ValueError(f'seed {seed} is not registered by {matrix["matrix_id"]}')
...
def default_work_dir(cfg: Config, seed: int) -> Path:
    return (ROOT / 'work_dirs/v2' / cfg.experiment.dataset_alias /
            cfg.experiment.method_id / f'trainseed_{seed}')
...
def _git_state() -> dict:
    ...
    status = subprocess.run(
        ...
        capture_output=True).stdout
    state = diff + b'\n--STATUS--\n' + status
    return dict(commit=commit, dirty=bool(status),
                dirty_diff_sha256=(hashlib.sha256(state).hexdigest()
                                      if status else None))


def write_resolution(cfg: Config, sources: dict, seed: int,
                     output_dir: str | Path | None = None) -> tuple[Path, Path]:
    output_dir = Path(output_dir) if output_dir else default_work_dir(cfg, seed)
    ...
    with tempfile.NamedTemporaryFile(
            mode='w', suffix='.py', dir=output_dir, delete=False) as stream:
    ...
    manifest = dict(
        ...
        selection=dict(split=cfg.experiment.selection_split,
...
def matrix_plan(matrix_path: str | Path) -> list[dict]:
    ...
    for method_name in matrix['methods']:
        cfg, _ = resolve_config(matrix_path, method_name,
                                matrix['training_seeds'][0])
        for seed in matrix['training_seeds']:
            records.append(dict(
                matrix_id=matrix['matrix_id'], dataset_id=cfg.dataset_id,
                method_name=method_name, method_id=cfg.experiment.method_id,
                config_id=cfg.experiment.config_id, training_seed=seed,
                parent_method_id=cfg.experiment.get('parent_method_id'),
                parent_checkpoint_required=cfg.experiment.get(
                    'parent_checkpoint_required', False),
                scientific_config_sha256=scientific_hash(cfg),
                work_dir=str(default_work_dir(cfg, seed).relative_to(ROOT)),
            ))
    return records
