...
def print_stats(s):
    ...
    if s['has_inf']:
        flag = " *** Inf! ***"
    print(f"  [{s['name']:30s}] shape={s['shape']} | "
...
for i in range(bs):
    ...
    x2 = (x1 + torch.rand(num_proposals, device=DEVICE) * 133 + 10).clamp(max=1333)
    y2 = (y1 + torch.rand(num_proposals, device=DEVICE) * 100 + 10).clamp(max=800)
    bboxes[i, :, 0] = x1
    bboxes[i, :, 1] = y1
    bboxes[i, :, 2] = x2
    bboxes[i, :, 3] = y2
...
proposals = roi_features.flatten(2).mean(-1).view(bs, num_proposals, feat_channels)
...
s_g1 = stats(gamma1, "gamma1"); print_stats(s_g1)
s_b1 = stats(beta1, "beta1"); print_stats(s_b1)
s_a1 = stats(alpha1, "alpha1"); print_stats(s_a1)
s_g2 = stats(gamma2, "gamma2"); print_stats(s_g2)

# --- Step 3d: Block 1 — Self-Attention with AdaLN ---
proposals_sa = proposals.view(bs, num_proposals, feat_channels).permute(1, 0, 2)
proposals_flat = proposals_sa.reshape(num_proposals * bs, feat_channels)
s = stats(proposals_flat, "before SA modulation"); print_stats(s)

q_modulated = F.layer_norm(proposals_flat, [feat_channels]) * (1 + gamma1) + beta1
q_modulated = q_modulated.view(num_proposals, bs, feat_channels)
s = stats(q_modulated, "q_modulated_SA"); print_stats(s)

attn_out, _ = single_head.self_attn(q_modulated, q_modulated, value=q_modulated)
s = stats(attn_out, "attn_out"); print_stats(s)
...
s = stats(proposals_flat, "after SA + alpha1 gate"); print_stats(s)

# --- Step 3e: Block 2 — DynamicConv ---
proposals_dc = proposals_sa.permute(1, 0, 2).reshape(1, bs * num_proposals, feat_channels)
roi_feat_for_dc = roi_features.view(bs * num_proposals, feat_channels, -1).permute(2, 0, 1)
s = stats(roi_feat_for_dc, "roi_feat → DynConv key/value"); print_stats(s)
s = stats(proposals_dc, "proposals → DynConv query"); print_stats(s)

inst_out = single_head.inst_interact(proposals_dc, roi_feat_for_dc)
s = stats(inst_out, "inst_interact_out"); print_stats(s)

proposals_dc = proposals_dc + single_head.dropout2(inst_out)
obj_features = single_head.norm2(proposals_dc)
s = stats(obj_features, "obj_features (after DynConv+LN)"); print_stats(s)

# --- Step 3f: Block 3 — FFN + AdaLN ---
obj_flat = obj_features.squeeze(0)
s = stats(obj_flat, "before FFN modulation"); print_stats(s)
ffn_input = F.layer_norm(obj_flat, [feat_channels]) * (1 + gamma2) + beta2
ffn_out = single_head.linear2(single_head.dropout(single_head.act(single_head.linear1(ffn_input))))
s = stats(ffn_out, "ffn_out"); print_stats(s)
obj_flat = obj_flat + alpha2 * ffn_out
s = stats(obj_flat, "fc_feature (after FFN+alpha2)"); print_stats(s)

# --- Step 3g: cls_head ---
class_logits = single_head.cls_head(obj_flat).float()
s = stats(class_logits, "class_logits"); print_stats(s)

# --- Step 3h: reg_head ---
pred_bboxes = single_head._predict_bboxes(obj_flat, bboxes)
s = stats(pred_bboxes, "pred_bboxes"); print_stats(s)
...
s = stats(features, "dyn_features (49 tokens)"); print_stats(s)
s = stats(parameters, "dyn_parameters"); print_stats(s)

param1, param2 = parameters.chunk(2, dim=1)
param1 = param1.view(-1, feat_channels, dyn.dynamic_dim)  # (N, 256, 64)
s = stats(param1, "dyn_param1"); print_stats(s)

features_bmm1 = torch.bmm(features, param1)  # (N, 49, 64)
s = stats(features_bmm1, "after bmm1"); print_stats(s)

features_n1 = dyn.norm1(features_bmm1)  # LayerNorm
s = stats(features_n1, "after LN1"); print_stats(s)

features_a1 = dyn.act(features_n1)  # ReLU
s = stats(features_a1, "after ReLU1"); print_stats(s)

param2 = param2.view(-1, dyn.dynamic_dim, feat_channels)  # (N, 64, 256)
s = stats(param2, "dyn_param2"); print_stats(s)

features_bmm2 = torch.bmm(features_a1, param2)  # (N, 49, 256)
s = stats(features_bmm2, "after bmm2"); print_stats(s)

features_n2 = dyn.norm2(features_bmm2)  # LayerNorm
s = stats(features_n2, "after LN2"); print_stats(s)

features_a2 = dyn.act(features_n2)  # ReLU
s = stats(features_a2, "after ReLU2"); print_stats(s)

features_flat = features_a2.reshape(features_a2.size(0), -1)  # (N, 49*256)
s = stats(features_flat, "dyn_flattened"); print_stats(s)

features_out = dyn.out_layer(features_flat)
s = stats(features_out, "dyn_out_layer"); print_stats(s)

features_n3 = dyn.norm3(features_out)
s = stats(features_n3, "dyn_out_LN3"); print_stats(s)

features_final = dyn.act(features_n3)
s = stats(features_final, "dyn_out_ReLU3"); print_stats(s)
...
print(f"noisy bboxes range: [{curr_bboxes.min():.1f}, {curr_bboxes.max():.1f}]")
...
for i in range(6):
    ...
    vel_s = stats(vel_seq[i], f"head_{i}_velocity") if vel_seq[i] is not None else None
    print_stats(cls_s)
    print_stats(bbox_s)
    if vel_s:
        print_stats(vel_s)
...
print("=" * 70)
if has_nan:
    print("*** RESULT: NaN FOUND in cascade — NaN propagates from earlier head ***")
else:
    print("*** RESULT: ALL CLEAN in cascade — NaN likely from LOSS computation ***")
