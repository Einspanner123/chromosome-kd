...
class DiffusionDetMatcher(nn.Module):
    ...
    def forward(
        ...
        if max_gt == 0:
            return [
                (
                    torch.zeros(
                        N, dtype=torch.bool, device=pred_bboxes.device
                    ),
                    torch.zeros(
                        N, dtype=torch.long, device=pred_bboxes.device
                    ),
                )
            ] * bs

        gt_bboxes_padded = pred_bboxes.new_zeros(bs, max_gt, 4)
        gt_labels_padded = pred_logits.new_full(
            (bs, max_gt), 0, dtype=torch.long
        )
        gt_num = []
        ...
        for i in range(bs):
            ...
            if n == 0:
                ...
                results.append(
                    self._dynamic_k_matching(
                        cost_matrix[i, :, :n], pairwise_ious[i, :, :n], n
                    )
                )
        return results
        ...
        if gt_cache is None:
            ...
            for i, t in enumerate(targets):
                ...
                if n > 0:
                    ...
                    gt_labels_padded[i, :n] = t.labels
                gt_ctrs = (
                    (t.bboxes[:, :2] + t.bboxes[:, 2:]) / 2
                    if n > 0
                    else t.bboxes.new_zeros(0, 2)
                )
                gt_wh = (
                    (t.bboxes[:, 2:] - t.bboxes[:, :2])
                    if n > 0
                    else t.bboxes.new_zeros(0, 2)
                )
                new_cache.append(
            ...
            gt_bboxes_padded = torch.stack(
                [c['gt_bboxes_padded'] for c in new_cache]
            )
            gt_labels_padded = torch.stack(
                [c['gt_labels_padded'] for c in new_cache]
            )
    ...
    def _batched_cost(
        ...
        gt_ctrs = (
            gt_bboxes_padded[:, :, :2] + gt_bboxes_padded[:, :, 2:]
        ) / 2  # [bs, max_gt, 2]
        gt_wh = (
            gt_bboxes_padded[:, :, 2:] - gt_bboxes_padded[:, :, :2]
        )  # [bs, max_gt, 2]
        ...
        lt = pred_ctrs.unsqueeze(2) - gt_bboxes_padded[:, :, :2].unsqueeze(
            1
        )  # [bs, N, max_gt, 2]
        rb = gt_bboxes_padded[:, :, 2:].unsqueeze(1) - pred_ctrs.unsqueeze(
            2
        )  # [bs, N, max_gt, 2]
        is_in_boxes = (
            torch.cat([lt, rb], dim=-1).min(-1)[0] > 0
        )  # [bs, N, max_gt]
        ...
        lt_c = pred_ctrs.unsqueeze(2) - center_tl.unsqueeze(
            1
        )  # [bs, N, max_gt, 2]
        rb_c = center_br.unsqueeze(1) - pred_ctrs.unsqueeze(
            2
        )  # [bs, N, max_gt, 2]
        is_in_centers = (
        ...
        is_in_boxes_anchor = is_in_boxes.any(2) | is_in_centers.any(
            2
        )  # [bs, N]
        ...
        for cost_fn in self.costs:
            ...
            if isinstance(cost_fn, FocalLossCost):
                cost_list.append(
                    self._batched_focal_cost(
                        cost_fn, pred_logits, gt_labels_padded, max_gt
                    )
                )
            elif isinstance(cost_fn, BBoxL1Cost):
                cost_list.append(
                    self._batched_l1_cost(
                        cost_fn, pred_bboxes, gt_bboxes_padded
                    )
                )
            else:
                # Fallback: per-image
                for i in range(bs):
                    ...
                    if n > 0:
                        cost_i = cost_fn(
                            pred_logits[i],
                            pred_bboxes[i],
                            gt_labels_padded[i, :n],
                            gt_bboxes_padded[i, :n],
                        )
                    else:
                    ...
                    if i == 0:
                        ...
                        batched_cost = torch.cat(
                            [batched_cost, cost_i.unsqueeze(0)], dim=0
                        )
        ...
        gt_valid_mask = torch.zeros(
            bs, 1, max_gt, dtype=torch.bool, device=pred_bboxes.device
        )
        for i in range(bs):
            if gt_num[i] > 0:
        ...
        cost_matrix[
            ~is_in_boxes_anchor.unsqueeze(-1).expand_as(cost_matrix)
        ] += 10000.0
    ...
    @staticmethod
    def _batched_focal_cost(
        cost_fn, pred_logits: Tensor, gt_labels: Tensor, max_gt: int
    ) -> Tensor:
        """批量 FocalLoss 代价: [bs, N, C] x [bs, M] -> [bs, N, M]"""
        num_classes = pred_logits.shape[-1]
        gt_labels_clamped = gt_labels.clamp(0, num_classes - 1)
        # Gather target logits: [bs, N, M]
        gt_labels_expanded = gt_labels_clamped.unsqueeze(1).expand(
            -1, pred_logits.shape[1], -1
        )
        ...
        neg_cost = (
            -(1 - cost_fn.alpha)
            * (p**cost_fn.gamma)
            * torch.log(1 - p + cost_fn.eps)
        )
        pos_cost = (
            -cost_fn.alpha
            * ((1 - p) ** cost_fn.gamma)
            * torch.log(p + cost_fn.eps)
        )
        return (pos_cost - neg_cost) * cost_fn.weight

    @staticmethod
    def _batched_l1_cost(
        cost_fn, pred_bboxes: Tensor, gt_bboxes: Tensor
    ) -> Tensor:
        ...
        return (pred.unsqueeze(2) - gt.unsqueeze(1)).abs().sum(
            -1
        ) * cost_fn.weight

    def _dynamic_k_matching(
        self, cost, pairwise_ious, num_gt
    ) -> Tuple[Tensor, Tensor]:
        ...
        _, top_rows = torch.topk(
            cost, candidate_topk, dim=0, largest=False
        )  # [candidate_topk, num_gt]
        gt_cols = (
            torch.arange(num_gt, device=cost.device)
            .unsqueeze(0)
            .expand(candidate_topk, -1)
        )
        # 创建 mask: row_idx < dynamic_ks[gt_idx]
        row_positions = torch.arange(
            candidate_topk, device=cost.device
        ).unsqueeze(1)  # [candidate_topk, 1]
        valid_mask = row_positions < dynamic_ks.unsqueeze(
            0
        )  # [candidate_topk, num_gt]
        ...
        matched_gt_inds = matching_matrix.argmax(
            1
        )  # [N] long (非 fg 位置无效，后续 mask)
